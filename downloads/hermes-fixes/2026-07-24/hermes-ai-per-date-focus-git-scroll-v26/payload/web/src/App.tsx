...
} from "@/lib/three-gold-shortcuts";
import {
  BRIEF_FULLSCREEN_STATE_MESSAGE_TYPES,
  PERSISTENT_FULLSCREEN_SHELL_ID,
  PersistentFullscreenContext,
  isBriefFullscreenToggleMessage,
  settlePersistentFullscreenShellScroll,
} from "@/lib/persistent-fullscreen";
...
const BUILTIN_NAV_REST: NavItem[] = [
  ...
  { path: "/brief-stock", label: "BRIEF-STOCK", icon: TrendingUp },
...
const THREE_GOLD_RETIRED_PLUGIN_NAMES = new Set([
  "brief-stock",
  "briefs-ai",
  "git-comments",
]);
...
export default function App() {
  ...
  const isPersistentBriefRoute = normalizedPath === "/briefs-ai" || normalizedPath === "/brief-stock";
  ...
  const togglePersistentFullscreen = useCallback(async () => {
    ...
    try {
      ...
      // Browser fullscreen denial leaves the dashboard route unchanged.
    }
  }, []);

  const publishPersistentFullscreenState = useCallback((active: boolean) => {
    document.querySelectorAll("iframe").forEach((iframe) => {
      BRIEF_FULLSCREEN_STATE_MESSAGE_TYPES.forEach((type) => {
        iframe.contentWindow?.postMessage({ type, active }, "*");
      });
    });
  }, []);
  ...
  useEffect(() => {
    const syncFullscreenState = () => {
      ...
      setIsPersistentFullscreen(active);
      publishPersistentFullscreenState(active);
    };
    const publishLoadedIframeState = (event: Event) => {
      ...
      if (active && shell && isPersistentBriefRoute) settlePersistentFullscreenShellScroll(shell);
    };
    document.addEventListener("fullscreenchange", syncFullscreenState);
    document.addEventListener("load", publishLoadedIframeState, true);
    syncFullscreenState();
    return () => {
      document.removeEventListener("fullscreenchange", syncFullscreenState);
      document.removeEventListener("load", publishLoadedIframeState, true);
    };
  }, [isPersistentBriefRoute, publishPersistentFullscreenState]);

  useEffect(() => {
    if (!isPersistentFullscreen || !isPersistentBriefRoute) return;
    const shell = fullscreenShellRef.current;
    if (!shell) return;
    const lockFullscreenShellScroll = () => {
      if (shell.scrollTop !== 0) shell.scrollTop = 0;
      if (shell.scrollLeft !== 0) shell.scrollLeft = 0;
    };
    lockFullscreenShellScroll();
    shell.addEventListener("scroll", lockFullscreenShellScroll, { passive: true });
    const frame = requestAnimationFrame(lockFullscreenShellScroll);
    return () => {
      cancelAnimationFrame(frame);
      shell.removeEventListener("scroll", lockFullscreenShellScroll);
    };
  }, [isPersistentBriefRoute, isPersistentFullscreen]);
  ...
  const customShortcutPaths = useMemo(
    () => sidebarNav.customItems.map((item) => item.path),
    [sidebarNav.customItems],
  );
  ...
  useEffect(() => {
    const navigateToCustomShortcut = (key: string): boolean => {
      const index = customTabIndexForShortcut(key, customShortcutPaths.length);
      if (index === null) return false;
      const pathname = customShortcutPaths[index];
      if (!pathname) return false;
      navigate({ pathname, search: location.search, hash: location.hash });
      return true;
    };

    const handleThreeGoldKeyboardShortcut = (event: KeyboardEvent) => {
      if (
        ...
        event.altKey ||
        event.shiftKey ||
        isThreeGoldKeyboardEditingTarget(event.target)
      ) {
        return;
      }
      if (event.key === "Enter" && customShortcutPaths.includes(normalizedPath)) {
        event.preventDefault();
        event.stopImmediatePropagation();
        void togglePersistentFullscreen();
        return;
      }
      if (navigateToCustomShortcut(event.key)) event.preventDefault();
    };

    const handleBriefIframeShortcut = (event: MessageEvent) => {
      const fromDashboardIframe = Array.from(document.querySelectorAll("iframe")).some(
        (iframe) => iframe.contentWindow === event.source,
      );
      if (!fromDashboardIframe) return;
      if (isBriefFullscreenToggleMessage(event.data?.type)) {
        void togglePersistentFullscreen();
        return;
      }
      if (event.data?.type !== CUSTOM_TAB_SHORTCUT_MESSAGE_TYPE) return;
      navigateToCustomShortcut(String(event.data.key ?? ""));
    };

    window.addEventListener("keydown", handleThreeGoldKeyboardShortcut, true);
    window.addEventListener("message", handleBriefIframeShortcut);
    return () => {
      window.removeEventListener("keydown", handleThreeGoldKeyboardShortcut, true);
      window.removeEventListener("message", handleBriefIframeShortcut);
    };
  }, [customShortcutPaths, location.hash, location.search, navigate, normalizedPath, togglePersistentFullscreen]);
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          ...
          >
            ...
            <nav
              ...
              aria-label={t.app.navigation}
            >
              {sidebarNav.customItems.length > 0 && (
                <div
          ...
          <PageHeaderProvider pluginTabs={pluginTabMeta}>
            ...
            >
              ...
              <div
                id={PERSISTENT_FULLSCREEN_SHELL_ID}
                ...
                className={cn(
                  ...
                  isPersistentFullscreen &&
                    (isPersistentBriefRoute
                      ? "h-dvh overflow-hidden bg-background-base p-0"
                      : "h-dvh overflow-y-auto overflow-x-hidden bg-background-base p-0"),
                )}
              >
                {normalizedPath === THREE_GOLD_GIT_WATCH_PATH && (
                  <Button
                    data-hermes-persistent-fullscreen-control
                    ghost
                    size="icon"
                    type="button"
                    className="absolute right-3 top-3 z-50"
                    onClick={() => void togglePersistentFullscreen()}
                    aria-label={isPersistentFullscreen ? "Exit GIT WATCH fullscreen" : "Enter GIT WATCH fullscreen"}
                    title={isPersistentFullscreen ? "Exit fullscreen" : "Enter fullscreen"}
                  >
                    {isPersistentFullscreen ? <Minimize2 /> : <Maximize2 />}
                  </Button>
                )}
                <PersistentFullscreenContext.Provider value={isPersistentFullscreen}>
                  <ProfileKeyedRoutes>
                    <Routes>
                ...
                </PersistentFullscreenContext.Provider>
