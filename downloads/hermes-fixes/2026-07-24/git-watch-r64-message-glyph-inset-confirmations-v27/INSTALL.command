...
PACKAGE_NAME="git-watch-r64-message-glyph-inset-confirmations-v27"
...
restore_failure() {
  ...
  if [[ "$TOUCHED" == "1" ]]; then
    if ! (cd "$BACKUP" && shasum -a 256 -c BACKUP-CHECKSUMS.sha256 >/dev/null); then
      echo "GIT_WATCH_R64_RESTORE_FAILED=backup-integrity" >&2
      return 1
    fi
    ...
    if [[ "$failed" != "0" ]]; then
      echo "GIT_WATCH_R64_RESTORE_FAILED=copy" >&2
      return 1
    fi
    echo "GIT_WATCH_R64_RESTORED_AFTER_FAILURE=$BACKUP" >&2
  fi
}
...
required_renderer=[
    ...
    'event.key !== "Tab"',
    '<meta name="git-watch-export-version" content="64">',
    '<meta name="git-watch-visual-baseline" content="64">',
    'Export format 64 · Visual baseline 64',
    ...
    '.git-comments-action-content{grid-column:1;display:grid;grid-template-columns:max-content;width:max-content;justify-self:start}',
    '.git-comments-action-message{grid-column:1;grid-row:1;margin:14px 0 24px;color:#cbd5e1;font-size:44px;line-height:1.2;font-weight:800;letter-spacing:.04em;white-space:nowrap}',
    '.git-comments-action-modal.red .git-comments-action-message{font-size:32px}',
    '.git-comments-action-buttons{grid-column:1;grid-row:2;justify-self:end;display:flex;align-items:center;gap:24px}',
    ...
    'e("div", { className: "git-comments-action-content" },',
    'padding:28px;font-family:"Alumni Sans SC",sans-serif',
    ...
    'CONFIRMATION DIALOG TOKENS (Revision 64)',
...
print('GIT_WATCH_R64_INSTALL_MARKERS=PASS')
PY
...
echo "GIT_WATCH_R64_BACKUP_RESTORE=PASS"
...
if [[ "${GIT_WATCH_R64_PAUSE_AFTER_RUNTIME:-0}" != "0" ]]; then
  [[ -z "${GIT_WATCH_R64_PAUSE_MARKER:-}" ]] || : > "$GIT_WATCH_R64_PAUSE_MARKER"
  sleep "$GIT_WATCH_R64_PAUSE_AFTER_RUNTIME"
fi

if [[ "${GIT_WATCH_R64_FAIL_AT:-}" == "after-runtime" ]]; then
...
TOUCHED=0
trap - EXIT
echo "GIT_WATCH_R64_RUNTIME_INSTALL=PASS"
