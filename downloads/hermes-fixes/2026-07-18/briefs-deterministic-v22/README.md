# Hermes Briefs deterministic templates v22

V22 is a cumulative update over V20. It preserves the normal-weight, exactly 11%-smaller Stock header SUMMARY value, titles the existing lifetime per-share column `TOTAL +/-`, adds an adjacent color-coded `DAY +/-` column using canonical daily per-share movement, and increases only the purchase date beneath each position by exactly 25% (`12.2px` → `15.25px`). Calculations, sorting, totals, and both CSV contracts remain unchanged.
...
```bash
chmod +x INSTALL_BRIEFS_DETERMINISTIC_V22.command
./INSTALL_BRIEFS_DETERMINISTIC_V22.command
```
...
Use the backup-specific rollback path printed by the installer. `ROLLBACK_LATEST_BRIEFS_DETERMINISTIC_V22.command` is also provided for convenience.
