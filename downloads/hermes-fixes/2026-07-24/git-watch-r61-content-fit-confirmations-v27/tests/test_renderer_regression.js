...
assert(exportedHtml.includes(exportClone.outerHTML) && exportedHtml.includes('<meta name="git-watch-export-version" content="61">') && exportedHtml.includes('<meta name="git-watch-visual-baseline" content="61">'), "export must embed the exact current HTML snapshot and Revision 61 export/visual metadata");
assert(exportedHtml.includes("<!-- GIT WATCH OFFLINE FILE GUIDE") && exportedHtml.includes("API-DEPENDENT SELECTION AND MUTATION CONTROLS OMITTED") && exportedHtml.includes("SUCCESS POPUP TOKENS (Revision 61)") && exportedHtml.includes("CONFIRMATION DIALOG TOKENS (Revision 61)") && exportedHtml.includes("font: embedded Alumni Sans SC ExtraBold 800") && exportedHtml.includes("border radius: 25px") && exportedHtml.includes("minimum width: 589.7103px") && exportedHtml.includes("minimum height: 245.1429px") && exportedHtml.includes("padding: 83.5714px 70.3257px") && exportedHtml.includes("text size: 60.54px") && exportedHtml.includes("letter spacing: 0.04em") && exportedHtml.includes("background fill alpha: 95%") && exportedHtml.includes("dwell: 2000ms, then fade: 500ms") && exportedHtml.includes("press master gain: 0.4 → 1.2 (3x amplitude)") && exportedHtml.includes("inline sound: Cuelume 0.1.2 success and data-cuelume-press"), "export must explain offline scope and the final Revision 61 popup/dialog/font tokens for readable future modification");
...
assert(source.includes('.git-comments-action-modal{width:max-content;max-width:calc(100vw - 64px);max-height:calc(100vh - 64px);overflow:auto;box-sizing:border-box;display:grid;grid-template-columns:max-content max-content;column-gap:24px;border:1px solid #475569;border-radius:25px;'), "confirmation panel must shrink to content and preserve viewport-safe scrolling");
...
assert(source.includes('font-family:"Alumni Sans SC",sans-serif') && source.includes('.git-comments-action-title{grid-column:1/-1;margin:0;color:#FFE6CB;font-size:78px;font-weight:800;letter-spacing:.04em;white-space:nowrap}') && source.includes('.git-comments-action-message{grid-column:1;grid-row:2;margin:14px 0 24px;color:#cbd5e1;font-size:51px;line-height:1.2;font-weight:800;letter-spacing:.04em;white-space:nowrap}') && source.includes('.git-comments-action-buttons{grid-column:2;grid-row:3;justify-self:start;display:flex;align-items:center;gap:24px}') && source.includes('.git-comments-action-buttons .git-comments-button{min-height:76px;padding:16px 28px;border-radius:18px;font-family:"Alumni Sans SC",sans-serif;font-size:24px;font-weight:800;white-space:nowrap}'), "confirmation title, message, and buttons must preserve Revision 61 typography and align to the content grid");
