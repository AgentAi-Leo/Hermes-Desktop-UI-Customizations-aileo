...
describe("brief helpers", () => {
  ...
  it("cycles adjacent cron dates forever at both archive boundaries", () => {
    ...
    expect(briefDateNavigationWraps(entries, "2026-07-13", "older")).toBe(true);
    expect(briefDateNavigationWraps(entries, "2026-07-14", "newer")).toBe(true);
    expect(briefDateNavigationWraps(entries, "2026-07-14", "older")).toBe(false);
    expect(briefDateNavigationWraps(entries, "2026-07-13", "newer")).toBe(false);
  });

  it("keeps the complete gold AI toolbar parent-owned through the oldest-to-latest wrap", () => {
    const briefsPage = readFileSync(new URL("../pages/BriefsPage.tsx", import.meta.url), "utf8");
    expect(briefsPage).toContain('data-hermes-ai-persistent-toolbar');
    expect(briefsPage).toContain('renderExportButtons("ai-persistent-toolbar")');
    expect(briefsPage).toContain('aria-label="Previous AI topic"');
    expect(briefsPage).toContain('aria-label={aiPlaying ? "Pause AI narration" : "Play AI narration"}');
    expect(briefsPage).toContain('aria-label="Next AI topic"');
    expect(briefsPage).toContain('aria-label="AI narration volume"');
    expect(briefsPage).toContain('aria-label="AI playback speed"');
    expect(briefsPage).toContain('aria-label="Load newer AI brief date"');
    expect(briefsPage).toContain('aria-label="Load older AI brief date"');
    expect(briefsPage).toContain('pendingAiViewportPositionRef.current = aiViewportByDateRef.current.get(targetDate)');
    expect(briefsPage).toContain('prepareBriefPreviewHtml(sourceHtml, kind, selected.generated_at, selected.date, kind === "ai")');

    const parentOwned = prepareBriefPreviewHtml(
      '<!doctype html><html><head></head><body><main><header><h1>AI Morning Brief</h1></header><article class="card"><h2>Topic</h2></article></main></body></html>',
      "ai",
      undefined,
      "2026-07-14",
      true,
  ...
  it("keeps one AI toolbar mounted when fullscreen state and date-loaded content change", () => {
    ...
    expect(briefsPage).toContain('prepareBriefPreviewHtml(sourceHtml, kind, selected.generated_at, selected.date, kind === "ai")');
    expect(briefsPage).not.toContain('selected.date, isPersistentFullscreen && kind === "ai"');
    expect(briefsPage).toContain('pendingAiViewportPositionRef.current = aiViewportByDateRef.current.get(targetDate)');
    expect(briefsPage).toContain('aiViewportByDateRef');
    expect(app).toContain('shell.addEventListener("scroll", lockFullscreenShellScroll');
    expect(app).toContain('shell.removeEventListener("scroll", lockFullscreenShellScroll');
  });
  ...
  it("restores each AI date to its own focused card and viewport", () => {
    const briefsPage = readFileSync(new URL("../pages/BriefsPage.tsx", import.meta.url), "utf8");
    ...
    expect(briefsPage).toContain("aiViewportByDateRef");
    expect(briefsPage).toContain("activeAiPreviewDateRef");
    expect(briefsPage).toContain("aiViewportByDateRef.current.set(activeDate");
    expect(briefsPage).toContain("topicIndex: Math.trunc(position.topicIndex)");
    expect(briefsPage).toContain("pendingAiViewportPositionRef.current = aiViewportByDateRef.current.get(targetDate)");
    expect(briefsPage).toContain("?? { scrollX: 0, scrollY: 0 }");
    ...
    expect(briefsPage).not.toContain('if (event.data?.type === "hermes-ai-viewport-state") {\n        return;');
  });
  ...
  it("adds a working fullscreen toggle immediately beside the Stock Brief title", () => {
    ...
    expect(app).toContain("await shell.requestFullscreen()");
  ...
  it("keeps one centered rail date and one canonical hero date without legacy duplicates", () => {
    const aiPreview = prepareBriefPreviewHtml(`<!doctype html><html><head></head><body><main>
      <header><span class="pill">July 16, 2026</span><h1>AI Morning Brief</h1><p class="date">July 16, 2026 · America/Los_Angeles</p></header>
    ...
    const document = new JSDOM(aiPreview).window.document;
    expect(document.querySelectorAll("#hermes-brief-player .hermes-date-nav-label")).toHaveLength(1);
    expect(document.querySelector("#hermes-brief-player .hermes-date-nav-label")?.textContent).toBe("July 16, 2026");
    expect(document.querySelectorAll("#hermes-ai-canonical > .hero > .pill")).toHaveLength(1);
    expect(document.querySelector("#hermes-ai-canonical > .hero > .pill")?.textContent).toBe("July 16, 2026");
  });

  it("keeps the stable route shell fullscreen for AI and republishes state after iframe loads", () => {
    const aiPreview = prepareBriefPreviewHtml(`<!doctype html><html><head></head><body><main>
      <header><span class="pill">July 16, 2026</span><h1>AI Morning Brief</h1><p class="date">July 16, 2026 · America/Los_Angeles</p></header>
  ...
  it("reserves parent-document Enter for the stable CUSTOM route shell", () => {
    const briefsPage = readFileSync(new URL("../pages/BriefsPage.tsx", import.meta.url), "utf8");
    const app = readFileSync(new URL("../App.tsx", import.meta.url), "utf8");
    expect(app).toContain('event.key === "Enter" && customShortcutPaths.includes(normalizedPath)');
    expect(app).toContain("event.stopImmediatePropagation()");
    expect(app).toContain("void togglePersistentFullscreen()");
  ...
  it("keeps one parent-owned AI volume level across sandboxed date iframe loads", () => {
    ...
    expect(briefsPage).toContain("const [aiVolume, setAiVolume] = useState(readStoredAiVolume)");
    expect(briefsPage).toContain("const aiVolumeRef = useRef(aiVolume)");
    ...
    expect(briefsPage).toContain('{ type: "hermes-ai-playback-rate-restore", playbackRate: aiPlaybackRateRef.current }');
  });
...
script:oldObfuscatedLink()">Obfuscated unsafe link</a>
  ...
  it("renders the complete AI player rail before older cron content can paint", () => {
    const result = prepareBriefPreviewHtml(`<!doctype html><html><head></head><body>
      <main><header class="hero"><span class="pill">July 18, 2026</span><h1>AI Morning Brief</h1></header>
      <section class="takeaways"><h2>FOUNDER TAKEAWAYS</h2><ol><li><strong>Older takeaway</strong>Detail</li></ol></section>
      <article class="card"><h2>1. Older topic</h2><p>Summary</p></article></main>
    </body></html>`, "ai", undefined, "2026-07-18");
    const document = new JSDOM(result).window.document;
    const bodyElements = Array.from(document.body.children) as HTMLElement[];

    expect(bodyElements[0]?.classList.contains("hermes-brief-player-placeholder")).toBe(true);
    expect(bodyElements[1]?.id).toBe("hermes-brief-player");
    expect(bodyElements[1]?.classList.contains("hermes-brief-player-sticky")).toBe(true);
    expect(bodyElements[2]?.id).toBe("hermes-ai-canonical");
    expect(document.querySelector("#hermes-brief-player #tts-prev")).not.toBeNull();
    expect(document.querySelector("#hermes-brief-player #tts-toggle")).not.toBeNull();
    expect(document.querySelector("#hermes-brief-player #tts-next")).not.toBeNull();
    expect(document.querySelector("#hermes-brief-player #tts-volume")).not.toBeNull();
    expect(document.querySelector("#hermes-brief-player #tts-speed")?.textContent).toBe("1.25x");
    expect(document.querySelector("#hermes-brief-player .hermes-date-nav-label")?.textContent).toBe("July 18, 2026");
    expect(document.querySelectorAll("#hermes-brief-player")).toHaveLength(1);
    expect(document.querySelector("#hermes-brief-player-controller")).not.toBeNull();
  });

  it("preserves Position Comparison when an already-canonical Stocks export is prepared again", () => {
    const canonicalStockHtml = `<!doctype html><html lang="en" data-hermes-canonical-stock="v26"><head></head><body id="hermes-stock-canonical">
      <header class="hero"><h1>Stock Brief</h1><span id="hermes-stock-date-pill" class="pill">July 20, 2026 - Mon.</span></header>
      <section id="hermes-portfolio-comparison"><h2>Portfolio Position Comparison</h2></section>
      <main id="hermes-stock-canonical-quotes" class="grid">
        <section class="hermes-stock-row" data-ticker="AAPL"><div class="hermes-stock-identity"><span class="hermes-stock-ticker">AAPL</span><span class="hermes-stock-change hermes-portfolio-negative">▼ -$7.15 (-2.14%)</span></div><div class="hermes-stock-current-price"><strong class="hermes-stock-price hermes-portfolio-negative">$326.59</strong></div><dl class="hermes-stock-metrics"><div><dt>Day High</dt><dd>$333.71</dd></div><div><dt>Day Low</dt><dd>$323.68</dd></div><div><dt>52-week High</dt><dd>$334.99</dd></div><div><dt>52-week Low</dt><dd>$201.50</dd></div><div><dt>Volume</dt><dd>53,468,000</dd></div></dl></section>
        <section class="hermes-stock-row" data-ticker="DIS"><div class="hermes-stock-identity"><span class="hermes-stock-ticker">DIS</span><span class="hermes-stock-change hermes-portfolio-negative">▼ -$1.26 (-1.29%)</span></div><div class="hermes-stock-current-price"><strong class="hermes-stock-price hermes-portfolio-negative">$96.41</strong></div><dl class="hermes-stock-metrics"><div><dt>Day High</dt><dd>$98.39</dd></div><div><dt>Day Low</dt><dd>$96.00</dd></div><div><dt>52-week High</dt><dd>$123.40</dd></div><div><dt>52-week Low</dt><dd>$92.19</dd></div><div><dt>Volume</dt><dd>9,861,700</dd></div></dl></section>
      </main></body></html>`;

    const result = prepareBriefPreviewHtml(canonicalStockHtml, "stock", undefined, "2026-07-20");
    const document = new JSDOM(result).window.document;
    const portfolioRows = Array.from(document.querySelectorAll("#hermes-portfolio-comparison tbody tr")) as HTMLTableRowElement[];
    ...
    expect(appleRow?.textContent).toContain("$326.59");
    expect(appleRow?.querySelector(".hermes-portfolio-day")?.textContent).toBe("-$7.15");
    expect(document.querySelectorAll('.hermes-stock-row[data-ticker="AAPL"] .hermes-stock-metrics > div')).toHaveLength(5);
  });
  ...
  it("locks the restored AI-focus and accepted Stock JavaScript controllers byte-for-byte", () => {
    ...
    expect(digest("PLAYER_CONTROLLER")).toBe("7cef120b6f0f69d3fb516e9571fc291b103512ef430160f40330ca25fcfcfb4c");
    expect(digest("STOCK_INTERACTION_CONTROLLER")).toBe("4b2840aa1b3935f84a2e3a6a2b72f08ec037991b8d82fea42a08669231012742");
  });
  ...
  it("forwards only unmodified numeric CUSTOM shortcuts from both brief iframes", () => {
    const source = '<html><body><main><article class="card"><h2>1. Topic</h2><p>Body.</p></article></main></body></html>';
    const previews = [
      [prepareBriefPreviewHtml(source, "ai"), "#hermes-brief-player-controller"],
      [prepareBriefPreviewHtml(source, "stock"), "#hermes-stock-interaction-controller"],
    ] as const;
    for (const [html, selector] of previews) {
      const controller = new JSDOM(html).window.document.querySelector(selector)?.textContent ?? "";
      const numericStart = controller.indexOf("if (/^[1-9]$/.test(event.key))");
      const messageAt = controller.indexOf('type: "hermes-custom-tab-shortcut", key: event.key', numericStart);
      const numericEnd = controller.indexOf("return;", messageAt);
      const numericHandler = controller.slice(numericStart, numericEnd);
      expect(numericStart).toBeGreaterThanOrEqual(0);
      expect(messageAt).toBeGreaterThan(numericStart);
      expect(numericHandler).toContain("event.metaKey || event.ctrlKey || event.altKey || event.shiftKey");
      expect(numericHandler).not.toContain('event.key === "["');
      expect(numericHandler).not.toContain('event.key === "]"');
    }
    expect(previews[0][0]).toContain('event.key === "[" ? "newer"');
    expect(previews[1][0]).toContain('event.key === "ArrowLeft" || event.key === "["');
  });
