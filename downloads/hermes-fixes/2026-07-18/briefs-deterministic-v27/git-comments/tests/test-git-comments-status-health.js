...
assert(source.includes('.git-comments-success{position:fixed;left:50%;top:50%;z-index:1100;width:min(1020px,calc(100vw - 96px));min-height:min(208px,calc(100vh - 96px));box-sizing:border-box;transform:translate(-50%,-50%);') && source.includes('display:flex;align-items:center;justify-content:center') && source.includes('padding:60px 96px') && source.includes('font-size:43.2px') && source.includes('border-radius:27px') && source.includes('box-shadow:0 24px 64px rgba(0,0,0,.6),0 8px 24px rgba(0,0,0,.36)') && source.includes('backdrop-filter:blur(8px)') && source.includes('transition:opacity .5s ease'), "center popup must match the yellow-guide 1020px by 208px box and reduce Revision 40 text exactly 10% from 48px to 43.2px while preserving all other styling");
