...
```bash
chmod +x INSTALL_BRIEFS_DETERMINISTIC_V24.command
./INSTALL_BRIEFS_DETERMINISTIC_V24.command
```
...
Use the backup-specific rollback path printed by the installer. `ROLLBACK_LATEST_BRIEFS_DETERMINISTIC_V24.command` is also provided for convenience.
