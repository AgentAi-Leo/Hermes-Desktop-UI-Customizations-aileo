...
assert(source.includes('showSuccess("URL SUCCESSFULLY ARCHIVED!", 3000, "cyan", "bottom")'), "archive success must publish the exact requested cyan text at bottom-center for three seconds before fading");
...
assert(source.includes('className: `git-comments-success ${successTone} ${successPlacement}${successFading ? " fading" : ""}`'), "success notice must render tone, viewport placement, and fading-state classes");
...
assert(source.includes('.git-comments-success{position:fixed;left:50%;z-index:1100;width:max-content;max-width:calc(100vw - 48px);transform:translateX(-50%);') && source.includes('border:1px solid #4ade80') && source.includes('transition:opacity .5s ease'), "timed success toast must be compact, horizontally centered, viewport-visible, and retain its green default and fade transition");
...
assert(String(archivedRows[0].children[1]?.props?.className || "").includes("git-comments-button unarchive") && String(archivedRows[0].children[2]?.props?.className || "").includes("git-comments-button delete"), "UNARCHIVE and DELETE must remain the right-side row actions");
...
assert(source.includes('.git-comments-success{position:fixed;left:50%;z-index:1100;width:max-content;max-width:calc(100vw - 48px);transform:translateX(-50%);') && source.includes('.git-comments-success.top{top:24px}') && source.includes('.git-comments-success.bottom{bottom:24px}'), "compact completion toasts must support top-center and bottom-center viewport placement");
assert(source.includes('.git-comments-success.cyan{border-color:#22d3ee;background:#083344;color:#cffafe}'), "archive/delete success notices must use cyan styling");
assert(source.includes('showSuccess("URL SUCCESSFULLY ARCHIVED!", 3000, "cyan", "bottom")'), "archive success must use the compact cyan bottom-center 3-second notice");
...
assert(source.includes('showSuccess("SUCCESSFULLY UNARCHIVED!", 3000, "green", "top")'), "unarchive must publish the exact green top-center success notice with one exclamation mark");
assert(!source.includes('SUCCESSFULLY UNARCHIVED!!'), "obsolete double-exclamation unarchive text must be removed");
assert(source.includes('git-comments-success ${successTone} ${successPlacement}') && source.includes('setSuccessTone(tone)') && source.includes('setSuccessPlacement(placement)'), "success renderer must apply requested tone and top/bottom placement");
...
assert(source.includes('const unarchive = async (id) => { if (await mutate("/watchlist/restore", { id })) showSuccess("SUCCESSFULLY UNARCHIVED!", 3000, "green", "top"); }'), "UNARCHIVE must use the archived restore endpoint and publish green feedback only after success");
