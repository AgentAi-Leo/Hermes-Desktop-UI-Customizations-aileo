...
describe("api.getPlugins", () => {
  it("sends the explicit management profile during initial plugin discovery", async () => {
    vi.stubGlobal("window", {});
    const fetchMock = jsonFetchMock([]);
    vi.stubGlobal("fetch", fetchMock);

    await api.getPlugins("local-ai-assist1");

    expect(fetchMock).toHaveBeenCalledWith(
      "/api/dashboard/plugins?profile=local-ai-assist1",
      expect.objectContaining({ credentials: "include" }),
    );
  });

  it("uses the current management profile when no explicit profile is supplied", async () => {
    vi.stubGlobal("window", {});
    const fetchMock = jsonFetchMock([]);
    vi.stubGlobal("fetch", fetchMock);
    setManagementProfile("profile with spaces");

    await api.getPlugins();

    expect(fetchMock).toHaveBeenCalledWith(
      "/api/dashboard/plugins?profile=profile%20with%20spaces",
      expect.objectContaining({ credentials: "include" }),
    );
  });
});
