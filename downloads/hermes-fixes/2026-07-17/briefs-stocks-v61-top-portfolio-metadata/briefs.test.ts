...
script:oldObfuscatedLink()">Obfuscated unsafe link</a>
  ...
  it("exports a self-contained full-page dashboard clone with archive context and player", () => {
    const result = briefDashboardHtml(
      '<html><head><title>AI Morning Brief</title></head><body><header class="hero"><span class="pill">AI Morning Brief</span><h1>AI Morning Brief — 2026-07-16</h1><p class="date">July 16, 2026 · America/Los_Angeles</p></header><main><div class="takeaways"><h2>Founder takeaways</h2></div><section><h2>Topic one</h2></section></main></body></html>',
      "ai",
      "2026-07-16",
    ...
    expect(result).toContain('"accepted_ui":"v52-export-manifest-parity"');
    ...
    expect(result).toContain('event.source !== window');
  ...
  it("exports one Founder section and exactly seven non-collapsed AI topics with a restore manifest", () => {
    ...
    try {
      ...
      expect(markdown).toContain('"accepted_ui":"v52-export-manifest-parity"');
  ...
  it("exports Stock with the shared date pill, undated title, dynamic time, archive-date links, and no player", () => {
    ...
    expect(result).toContain('"accepted_ui":"v61-top-portfolio-metadata"');
  ...
  it("renders the yellow date pill from the exact live July 16 header shape", () => {
    ...
    expect(result).toContain("Generated end-of-day regular-market snapshot after the U.S. close · USD");
  ...
  it("selects, focuses, scrolls, highlights, and speaks the exact clicked card", () => {
    ...
    expect(result).toContain('founderTakeaways.scrollIntoView({ behavior: "smooth", block: "start" });');
