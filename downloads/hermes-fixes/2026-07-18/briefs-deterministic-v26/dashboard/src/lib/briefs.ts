...
function canonicalAiDocument(html: string, selectedDate?: string): string {
  ...
  const activeDatePill = date ? `<span class="hermes-ai-active-date-pill" role="status" aria-label="AI brief date: ${escape(date)}">${escape(date)}</span>` : "";
  const takeaways = hasTakeaways ? `<section class="takeaways"><div class="hermes-takeaways-heading-row"><h2>FOUNDER TAKEAWAYS</h2>${activeDatePill}</div>${takeawayList}</section>` : "";
