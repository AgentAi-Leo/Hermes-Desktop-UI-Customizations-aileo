...
BACKUP="$HOME/.hermes/backups/briefs-deterministic-v19-$STAMP"
LOG="$HOME/.hermes/logs/briefs-deterministic-v19-${PORT}.log"
...
for marker in (b'v61-date-load-keyboard-focus',b'v74-ticker-above-company',b'hermes-stock-portfolio-summary',b'SUMMARY',b'v16-canonical-stock-wrapper-spacing'):
...
if not re.search(rb'\.hermes-stock-summary-value\s*\{[^}]*font-weight:\s*400\s*!important', data):
    raise SystemExit('Live bundle missing normal-weight Stock SUMMARY value: font-weight 400')
print('LIVE_BUNDLE_MARKERS=PASS')
PY
fi

cat > "$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V19.command" <<EOF
#!/usr/bin/env bash
set -euo pipefail
PREVIEW="${PREVIEW}"
...
echo "ROLLBACK_BRIEFS_DETERMINISTIC_V19=PASS"
EOF
chmod +x "$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V19.command"

trap - EXIT
echo "BRIEFS_DETERMINISTIC_V19_INSTALL=PASS"
...
echo "BACKUP=$BACKUP"
echo "ROLLBACK=$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V19.command"
echo "PREVIEW=http://127.0.0.1:$PORT/briefs-stocks?profile=$PROFILE&candidate=deterministic-v19"
