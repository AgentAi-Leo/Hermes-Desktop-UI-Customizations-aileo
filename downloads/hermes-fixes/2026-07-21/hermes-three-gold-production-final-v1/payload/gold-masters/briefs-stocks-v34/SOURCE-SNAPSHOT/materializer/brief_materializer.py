...
AI_ROOT = Path(os.environ.get("HERMES_AI_BRIEFS_ROOT", str(Path.home() / ".hermes" / "zDownloads" / "__AI-DAILY-BRIEFS")))
STOCK_ROOT = Path(os.environ.get("HERMES_STOCK_BRIEFS_ROOT", str(Path.home() / ".hermes" / "zDownloads" / "_STOCK-BRIEFS")))
...
def validate_artifacts(mode: str, rendered: RenderedBrief, files: dict[str, bytes]) -> None:
    ...
    if mode == "ai":
        ...
        if b"ticker,current_price,daily_change,daily_change_pct,Agent\n" != files["csv"].splitlines(keepends=True)[0]:
...
def publish(mode: str) -> int:
    ...
    if state.get("schema") == 4 and state.get("session_id") == session_id:
        return 0
