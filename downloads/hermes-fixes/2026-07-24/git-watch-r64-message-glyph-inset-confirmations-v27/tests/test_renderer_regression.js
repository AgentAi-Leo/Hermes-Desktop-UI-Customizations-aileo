...
const exportClone = {
  ...
  querySelectorAll(selector) {
    assert.strictEqual(selector, ".git-comments-panel-add,.git-comments-success,.git-comments-error,.git-comments-button.add-toggle,.git-comments-button.archive,.git-comments-button.delete,.git-comments-button.restore,.git-comments-button.view-archived,.git-comments-button.retry-connection,.git-comments-button.export-html,.git-comments-archive-bulk-controls,.git-comments-archive-select,.git-comments-action-backdrop");
...
assert(exportedHtml.includes(exportClone.outerHTML) && exportedHtml.includes('<meta name="git-watch-export-version" content="64">') && exportedHtml.includes('<meta name="git-watch-visual-baseline" content="64">'), "export must embed the exact current HTML snapshot and Revision 64 export/visual metadata");
assert(exportedHtml.includes("<!-- GIT WATCH OFFLINE FILE GUIDE") && exportedHtml.includes("API-DEPENDENT SELECTION AND MUTATION CONTROLS OMITTED") && exportedHtml.includes("SUCCESS POPUP TOKENS (Revision 64)") && exportedHtml.includes("CONFIRMATION DIALOG TOKENS (Revision 64)") && exportedHtml.includes("font: embedded Alumni Sans SC ExtraBold 800") && exportedHtml.includes("border radius: 25px") && exportedHtml.includes("minimum width: 589.7103px") && exportedHtml.includes("minimum height: 245.1429px") && exportedHtml.includes("padding: 83.5714px 70.3257px") && exportedHtml.includes("text size: 60.54px") && exportedHtml.includes("letter spacing: 0.04em") && exportedHtml.includes("background fill alpha: 95%") && exportedHtml.includes("dwell: 2000ms, then fade: 500ms") && exportedHtml.includes("press master gain: 0.4 → 1.2 (3x amplitude)") && exportedHtml.includes("inline sound: Cuelume 0.1.2 success and data-cuelume-press"), "export must explain offline scope and the final Revision 64 popup/dialog/font tokens for readable future modification");
...
assert(!exportedHtml.includes('<meta name="git-watch-export-version" content="52">') && !exportedHtml.includes('<meta name="git-watch-visual-baseline" content="52">'), "export must not retain superseded Revision 52 markers");
...
assert(source.includes('if (event.key !== "Enter" || addOpen || busy || state.loading || archiveView || actionDialog || event.defaultPrevented'), "Enter-to-launch must run only while the form, archive reader, and action confirmation are closed and the view is idle");
...
assert(archivedRows.length === 1 && nodes(archivedRows[0], (node) => String(node.props?.className || "").includes("git-comments-button unarchive")).length === 0, "archived row must not retain the superseded single UNARCHIVE button");
assert(archivedRows.length === 1 && nodes(archivedRows[0], (node) => String(node.props?.className || "").includes("git-comments-button delete")).length === 0, "archived row must not retain the superseded single DELETE button");
...
assert(String(archivedRows[0].children[0]?.props?.className || "") === "git-comments-archive-select", "archived row must begin with its selection checkbox");
assert(String(archivedRows[0].children[1]?.props?.className || "") === "git-comments-archived-content", "archived identity and summary must occupy the flexible content column after selection");
assert(archivedRows[0].children.length === 2, "archived row must contain only its selection checkbox and flexible content column");
const archivedPrimary = archivedRows[0].children[1]?.children?.[0];
...
assert(!source.includes('.git-comments-archived-actions{') && !source.includes('className: "git-comments-archived-actions"'), "retired per-row archived action group must be removed from CSS and DOM");
...
assert(source.includes('showSuccess("SUCCESSFULLY DELETED!", 2000, "red")'), "active single-delete must retain the red SUCCESSFULLY DELETED popup");
assert(!source.includes('showSuccess("URL ADDED SUCCESSFULLY!", 3000)') && !source.includes(', 3000,'), "all successful action paths must retain the unified 2000ms dwell");
assert(source.includes('showSuccess(`${count} URL${count === 1 ? "" : "S"} SUCCESSFULLY UNARCHIVED!`, 2000, "green")'), "bulk unarchive must publish the exact green unified-center success popup");
...
assert(source.includes('mutate("/watchlist/delete", { id: dialog.id })'), "DELETE action must call the permanent-delete endpoint through the shared dispatcher");
assert(source.includes("Permanently delete this watched URL?") && !source.includes("window.confirm("), "DELETE action must require an in-app confirmation without native browser dialogs");
assert(!source.includes('const unarchive = async (id)'), "single archived UNARCHIVE handler must be removed now that selection is authoritative");
assert(!source.includes('const deleteArchived = (id, event)'), "single archived DELETE handler must be removed now that selection is authoritative");
assert(!source.includes('kind: "delete-archived"'), "single archived DELETE dispatcher branch must be removed");
assert(source.includes('.git-comments-action-modal{width:max-content;max-width:min(869px,calc(100vw - 64px));max-height:calc(100vh - 64px);overflow:auto;box-sizing:border-box;display:grid;grid-template-columns:max-content;border:1px solid #475569;border-radius:25px;'), "confirmation panel must shrink to one content column, respect the screenshot-C cap, and preserve viewport-safe scrolling");
assert(!source.includes('.git-comments-action-modal{width:min(620px,calc(100vw - 32px));padding:22px}'), "rejected Revision 57 mobile width override must remain absent");
assert(source.includes('font-family:"Alumni Sans SC",sans-serif') && source.includes('.git-comments-action-title{grid-column:1;margin:0;color:#FFE6CB;font-size:68px;font-weight:800;letter-spacing:.04em;white-space:nowrap}') && source.includes('.git-comments-action-content{grid-column:1;display:grid;grid-template-columns:max-content;width:max-content;justify-self:start}') && source.includes('.git-comments-action-message{grid-column:1;grid-row:1;margin:14px 0 24px;color:#cbd5e1;font-size:44px;line-height:1.2;font-weight:800;letter-spacing:.04em;white-space:nowrap}') && source.includes('.git-comments-action-buttons{grid-column:1;grid-row:2;justify-self:end;display:flex;align-items:center;gap:24px}') && source.includes('.git-comments-action-buttons .git-comments-button{min-height:76px;padding:16px 28px;border-radius:18px;font-family:"Alumni Sans SC",sans-serif;font-size:24px;font-weight:800;white-space:nowrap}') && source.includes('e("div", { className: "git-comments-action-content" },'), "confirmation title, message, and adjacent buttons must preserve typography while the inner content grid aligns actions to the actual message box");
...
assert(!source.includes('grid-template-columns:max-content max-content') && !source.includes('.git-comments-action-buttons{grid-column:2'), "Revision 64 must not regress to the R62 second action column");
