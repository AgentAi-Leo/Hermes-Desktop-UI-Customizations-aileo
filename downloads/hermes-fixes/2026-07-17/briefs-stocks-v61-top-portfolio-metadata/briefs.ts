...
.hermes-stock-row { grid-column: 1 / -1 !important; width: 100% !important; max-width: none !important; box-sizing: border-box !important; display: grid !important; grid-template-columns: minmax(280px, 1.65fr) repeat(5, minmax(104px, 1fr)) !important; grid-template-rows: auto auto !important; gap: 6px 12px !important; align-items: center !important; margin: 0 !important; padding: 16px 22px !important; border: 1px solid var(--line, #334155) !important; border-radius: 16px !important; background: var(--panel, #111827) !important; box-shadow: none !important; }
.hermes-stock-identity { display: contents !important; }
.hermes-stock-title { grid-column: 1 !important; grid-row: 1 !important; display: flex !important; align-items: baseline !important; flex-wrap: wrap !important; gap: 0 10px !important; min-width: 0 !important; }
.hermes-stock-ticker { color: var(--text, #f4f7fb) !important; font-size: 1.55rem !important; font-weight: 850 !important; letter-spacing: .02em !important; }
.hermes-stock-company { color: var(--text, #f4f7fb) !important; font-size: 1.1rem !important; font-weight: 700 !important; }
.hermes-stock-change { font-size: 1.1rem !important; font-weight: 800 !important; white-space: nowrap !important; }
.hermes-stock-price { grid-column: 1 !important; grid-row: 2 !important; color: #ffe08a !important; font-size: 1.84rem !important; line-height: 1 !important; font-variant-numeric: tabular-nums !important; }
.hermes-stock-metrics { display: contents !important; margin: 0 !important; }
.hermes-stock-metrics > div { min-width: 0 !important; }
.hermes-stock-metrics > div:nth-child(1) { grid-column: 2 !important; grid-row: 1 / span 2 !important; }
.hermes-stock-metrics > div:nth-child(2) { grid-column: 3 !important; grid-row: 1 / span 2 !important; }
.hermes-stock-metrics > div:nth-child(3) { grid-column: 4 !important; grid-row: 1 / span 2 !important; }
.hermes-stock-metrics > div:nth-child(4) { grid-column: 5 !important; grid-row: 1 / span 2 !important; }
.hermes-stock-metrics > div:nth-child(5) { grid-column: 6 !important; grid-row: 1 / span 2 !important; }
.hermes-stock-metrics dt { color: var(--muted, #aab8ca) !important; font-size: .9625rem !important; font-weight: 650 !important; letter-spacing: .04em !important; line-height: 1.2 !important; text-transform: uppercase !important; }
.hermes-stock-metrics dd { margin: 4px 0 0 !important; color: var(--text, #f4f7fb) !important; font-size: 1.4375rem !important; font-weight: 800 !important; font-variant-numeric: tabular-nums !important; }
@media (max-width: 1040px) { .hermes-stock-row { grid-template-columns: minmax(240px, 1fr) repeat(3, minmax(104px, 1fr)) !important; } .hermes-stock-metrics > div:nth-child(1) { grid-column: 2 !important; grid-row: 1 !important; } .hermes-stock-metrics > div:nth-child(2) { grid-column: 3 !important; grid-row: 1 !important; } .hermes-stock-metrics > div:nth-child(3) { grid-column: 4 !important; grid-row: 1 !important; } .hermes-stock-metrics > div:nth-child(4) { grid-column: 2 !important; grid-row: 2 !important; } .hermes-stock-metrics > div:nth-child(5) { grid-column: 3 !important; grid-row: 2 !important; } }
@media (max-width: 680px) { .hermes-stock-row { grid-template-columns: 1fr 1fr !important; gap: 12px !important; } .hermes-stock-title, .hermes-stock-price { grid-column: 1 / -1 !important; } .hermes-stock-price { grid-row: 2 !important; } .hermes-stock-metrics > div { grid-column: auto !important; grid-row: auto !important; } }
</style>`;
...
@media (max-width: 900px) { body > header h1, body > main > header h1, .wrap > header h1, .hero h1 { font-size: 48px !important; } }
</style>`;
...
(() => {
  ...
  function normalizeAiVisualStructure() {
    ...
    if (heading) heading.textContent = heading.textContent.replace(/\\s*(?:—|-)\\s*\\d{4}-\\d{2}-\\d{2}\\s*$/, "");
  }
  ...
  function ensureDateNavigator() {
    ...
    navigator.querySelector("#hermes-date-newer")?.addEventListener("click", () => {
      window.parent?.postMessage({ type: DATE_NAV_MESSAGE_TYPE, direction: "newer" }, "*");
    });
    navigator.querySelector("#hermes-date-older")?.addEventListener("click", () => {
      window.parent?.postMessage({ type: DATE_NAV_MESSAGE_TYPE, direction: "older" }, "*");
    });
  }
  ...
  function selectAndPlay(selectedIndex) {
  ...
  function playFounderTakeaways() {
    ...
    founderTakeaways.scrollIntoView({ behavior: "smooth", block: "start" });
  ...
  function init() {
    ...
    window.addEventListener("message", (event) => {
      if (event.source !== window.parent || event.data?.type !== MESSAGE_TYPE) return;
...
function stockPortfolioComparison(html: string): string {
  ...
  const totalReturn = totalBasis ? (totalGain / totalBasis) * 100 : 0;
...
function normalizeStockQuoteRows(html: string): string {
  ...
  const cardPattern = /<(section|article)\b(?=[^>]*\bclass\s*=\s*(?:"[^"]*\bcard\b[^"]*"|'[^']*\bcard\b[^']*'))([^>]*)>([\s\S]*?)<\/\1\s*>/gi;

  return html.replace(cardPattern, (source, _tag: string, attributes: string, body: string) => {
    if (!/\bclass\s*=\s*(?:"[^"]*\bcard\b[^"]*"|'[^']*\bcard\b[^']*')/i.test(attributes)) return source;
    const tickerMatch = text(body.match(/<(?:div|span|h[1-6])\b[^>]*class\s*=\s*(?:"[^"]*\bticker\b[^"]*"|'[^']*\bticker\b[^']*')[^>]*>([\s\S]*?)<\/(?:div|span|h[1-6])\s*>/i)?.[1] ?? body.match(/<h[1-6]\b[^>]*>\s*(AAPL|AMZN|NVDA|SNAP|GOOGL|MSFT|DIS)\b/i)?.[1] ?? "").match(/(AAPL|AMZN|NVDA|SNAP|GOOGL|MSFT|DIS)/i);
    ...
    const company = text(body.match(/<(?:div|span|p)\b[^>]*class\s*=\s*(?:"[^"]*\bcompany\b[^"]*"|'[^']*\bcompany\b[^']*')[^>]*>([\s\S]*?)<\/(?:div|span|p)\s*>/i)?.[1] ?? "").replace(/\bCorporation\b/gi, "Co.");
    const dailyChange = text(body.match(/<(?:div|span|p)\b[^>]*class\s*=\s*(?:"[^"]*\bchange\b[^"]*"|'[^']*\bchange\b[^']*')[^>]*>([\s\S]*?)<\/(?:div|span|p)\s*>/i)?.[1] ?? "");
    ...
    const metrics = Array.from(body.matchAll(/<dt\b[^>]*>([\s\S]*?)<\/dt\s*>\s*<dd\b[^>]*>([\s\S]*?)<\/dd\s*>/gi))
      .slice(0, 5)
      .map((entry) => `<div><dt>${escape(text(entry[1]))}</dt><dd>${escape(text(entry[2]))}</dd></div>`)
      .join("");

    const priceClass = changeClass ? ` ${changeClass}` : "";
    return `<section class="hermes-stock-row" data-ticker="${ticker}"><div class="hermes-stock-identity"><div class="hermes-stock-title"><span class="hermes-stock-ticker">${ticker}</span>${company ? `<span class="hermes-stock-company">${escape(company)}</span>` : ""}${dailyChange ? `<span class="hermes-stock-change ${changeClass}">${escape(dailyChange)}</span>` : ""}</div><strong class="hermes-stock-price${priceClass}">${escape(price)}</strong></div><dl class="hermes-stock-metrics">${metrics}</dl></section>`;
  });
}
...
function injectStockPortfolioComparison(html: string): string {
  ...
  const dateMatch = html.match(/\bdate\s*:\s*(\d{4}-\d{2}-\d{2})/i) ?? html.match(/Stock Brief\s*[—-]\s*(\d{4}-\d{2}-\d{2})/i);
...
function normalizeStockArchive(html: string): string {
  const withoutPortfolioComparison = html.replace(
    /<section\b(?=[^>]*(?:\bid\s*=\s*(?:"hermes-portfolio-comparison"|'hermes-portfolio-comparison')|\baria-label\s*=\s*(?:"[^"]*Portfolio(?: Position)? Comparison[^"]*"|'[^']*Portfolio(?: Position)? Comparison[^']*')))[^>]*>[\s\S]*?<\/section\s*>/gi,
    "",
  );
  return withoutPortfolioComparison;
}
...
function normalizeStockHeroMarkup(html: string, generatedAt?: number): string {
  ...
  const titleDate = stockHeader.match(
    /<h1\b[^>]*>\s*Stock Brief\s*(?:—|&mdash;|&#8212;|-)\s*(\d{4}-\d{2}-\d{2})\s*<\/h1\s*>/i,
  )?.[1] ?? stockHeader.match(/\b(?:Los\s+Angeles\s+)?date\s*:\s*(\d{4}-\d{2}-\d{2})\b/i)?.[1] ?? "";
  const timezone = stockHeader.match(/\b([A-Za-z_]+\/[A-Za-z_]+)\b/)?.[1] ?? "";
  const generationTime = formatArchiveGenerationTime(generatedAt, timezone);
  ...
  if (/End-of-day regular-market snapshot after the U\.S\. close\s*·\s*USD/i.test(stockHeader.replace(/<[^>]+>/g, " "))) {
    normalized = normalized.replace(
      /<(div|p|span)\b(?=[^>]*\bclass\s*=\s*(?:"[^"]*\bsub\b[^"]*"|'[^']*\bsub\b[^']*'))[^>]*>[\s\S]*?<\/\1\s*>/i,
      '<div class="sub">Generated end-of-day regular-market snapshot after the U.S. close · USD</div>',
    );
  }
...
export const BRIEF_EXPORT_VERSION = "v52-export-manifest-parity";
export const STOCK_BRIEF_EXPORT_VERSION = "v61-top-portfolio-metadata";
...
export function prepareBriefPreviewHtml(html: string, kind?: BriefKind, generatedAt?: number): string {
  ...
  const aiArchiveWithHero = kind === "ai" ? normalizeAiHeroMarkup(aiArchiveWithoutAgenda, generatedAt) : aiArchiveWithoutAgenda;
  const aiArchiveWithHeadlines = kind === "ai" ? normalizeAiTakeawayHeadlines(aiArchiveWithHero) : aiArchiveWithHero;
  const stockArchiveWithoutProducerComparison = kind === "stock" ? normalizeStockArchive(aiArchiveWithHeadlines) : aiArchiveWithHeadlines;
  ...
  const withArchiveStyle = kind === "ai"
    ...
    : kind === "stock"
      ...
      : withPlayerStyle;
  return kind === "stock" ? withArchiveStyle : injectBeforeClosingTag(withArchiveStyle, "body", PLAYER_CONTROLLER);
}
...
export function briefDashboardHtml(
  ...
  const shellStyle = `<style id="hermes-brief-export-shell-style">
...
export function briefDashboardMarkdown(html: string, kind: BriefKind, generatedAt?: number): string {
  ...
  if (kind === "stock") {
    document.querySelectorAll(".hermes-stock-row").forEach((row) => {
      ...
      lines.push("", `## ${ticker}${company ? ` — ${company}` : ""}`, price, metrics);
    });
  } else {
