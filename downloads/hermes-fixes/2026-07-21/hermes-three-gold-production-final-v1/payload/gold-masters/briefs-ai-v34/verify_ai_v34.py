...
def require(ok,msg):
    if not ok: raise SystemExit('AI_V34_SPLIT_VERIFY=FAIL: '+msg)

exports=ROOT/'EXPORTS'
expected={*(f'BRIEFS-AI - {d}.html' for d in DATES),*(f'AI Morning Brief - {d}.md' for d in DATES)}
...
for d in DATES:
    text=(exports/f'BRIEFS-AI - {d}.html').read_text()
    styles=re.findall(r'<style[^>]*>(.*?)</style>',text,re.I|re.S)
    scripts=re.findall(r'<script[^>]*>(.*?)</script>',text,re.I|re.S)
    require(len(styles)==5,f'{d}: expected 5 inline style blocks')
    require(len(scripts)==2,f'{d}: expected 2 inline script blocks')
    ...
    for sibling in DATES:
        require(f'BRIEFS-AI - {sibling}.html' in text,f'{d}: sibling navigation missing {sibling}')
    md=(exports/f'AI Morning Brief - {d}.md').read_text()
...
require(audit.get('passed') is True and audit.get('comparison_count')==25,'byte identity audit is not 25/25')
require(len(list((ROOT/'REFERENCES'/'CANONICAL').glob('*')))==10,'canonical reference count is not 10')
...
print('BRIEFS_AI_V34_SPLIT_VERIFY=PASS')
print('HTML=5 MARKDOWN=5 BYTE_IDENTITY=25/25 CYCLIC_NAVIGATION=PASS')
