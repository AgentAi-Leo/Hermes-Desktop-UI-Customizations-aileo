...
node tests/test_renderer_r63.js
node tests/test_visual_r63.js
if [[ "${GIT_WATCH_R63_SKIP_SURFACE_MUTANTS:-0}" != "1" ]]; then bash tests/test_surface_mutants_r63.sh; fi
...
print('GIT_WATCH_R63_API_SYNTAX=PASS')
PY
"$PYTHON_BIN" tests/test_bulk_api.py
bash tests/test_installer_r63.sh
if [[ "${GIT_WATCH_R63_SKIP_MACOS_COMPAT:-0}" != "1" ]]; then bash tests/test_macos_compat_r63.sh; fi
bash -n INSTALL.command
bash -n tests/github-comments-checker-v27-review.sh
echo "GIT_WATCH_R63_PACKAGE_VERIFICATION=PASS"
