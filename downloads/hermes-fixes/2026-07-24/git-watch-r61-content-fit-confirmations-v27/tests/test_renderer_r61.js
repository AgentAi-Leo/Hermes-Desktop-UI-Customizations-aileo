...
assert(source.includes('<meta name="git-watch-export-version" content="61">'), 'export schema must advance to Revision 61');
assert(source.includes('<meta name="git-watch-visual-baseline" content="61">'), 'visual baseline must identify Revision 61');
assert(source.includes('Export format 61 · Visual baseline 61'), 'visible export provenance must identify Revision 61');
...
assert(source.includes('CONFIRMATION DIALOG TOKENS (Revision 61)'), 'offline guide must document the rounded Alumni Sans SC confirmation dialog');
...
assert(source.includes('title: 78px; message: 51px; action buttons: 76px minimum height / 24px labels'), 'export guide must document Revision 61 dialog scaling');
assert(source.includes('archived bulk colors: DESELECT ALL yellow; UNARCHIVE SELECTED green'), 'export guide must document Revision 61 semantic bulk colors');
...
console.log('GIT_WATCH_R61_RENDERER_CONTRACT=PASS');
