...
assert(exportedHtml.includes(exportClone.outerHTML) && exportedHtml.includes('<meta name="git-watch-export-version" content="63">') && exportedHtml.includes('<meta name="git-watch-visual-baseline" content="63">'), "export must embed the exact current HTML snapshot and Revision 63 export/visual metadata");
assert(exportedHtml.includes("<!-- GIT WATCH OFFLINE FILE GUIDE") && exportedHtml.includes("API-DEPENDENT SELECTION AND MUTATION CONTROLS OMITTED") && exportedHtml.includes("SUCCESS POPUP TOKENS (Revision 63)") && exportedHtml.includes("CONFIRMATION DIALOG TOKENS (Revision 63)") && exportedHtml.includes("font: embedded Alumni Sans SC ExtraBold 800") && exportedHtml.includes("border radius: 25px") && exportedHtml.includes("minimum width: 589.7103px") && exportedHtml.includes("minimum height: 245.1429px") && exportedHtml.includes("padding: 83.5714px 70.3257px") && exportedHtml.includes("text size: 60.54px") && exportedHtml.includes("letter spacing: 0.04em") && exportedHtml.includes("background fill alpha: 95%") && exportedHtml.includes("dwell: 2000ms, then fade: 500ms") && exportedHtml.includes("press master gain: 0.4 → 1.2 (3x amplitude)") && exportedHtml.includes("inline sound: Cuelume 0.1.2 success and data-cuelume-press"), "export must explain offline scope and the final Revision 63 popup/dialog/font tokens for readable future modification");
...
assert(source.includes('font-family:"Alumni Sans SC",sans-serif') && source.includes('.git-comments-action-title{grid-column:1;margin:0;color:#FFE6CB;font-size:68px;font-weight:800;letter-spacing:.04em;white-space:nowrap}') && source.includes('.git-comments-action-message{grid-column:1;grid-row:2;margin:14px 0 24px;color:#cbd5e1;font-size:44px;line-height:1.2;font-weight:800;letter-spacing:.04em;white-space:nowrap}') && source.includes('.git-comments-action-buttons{grid-column:1;grid-row:3;justify-self:end;display:flex;align-items:center;gap:24px}') && source.includes('.git-comments-action-buttons .git-comments-button{min-height:76px;padding:16px 28px;border-radius:18px;font-family:"Alumni Sans SC",sans-serif;font-size:24px;font-weight:800;white-space:nowrap}'), "confirmation title, message, and buttons must preserve Revision 63 typography while action right edge stays inside the message column");
...
assert(!source.includes('grid-template-columns:max-content max-content') && !source.includes('.git-comments-action-buttons{grid-column:2'), "Revision 63 must not regress to the R62 second action column");
