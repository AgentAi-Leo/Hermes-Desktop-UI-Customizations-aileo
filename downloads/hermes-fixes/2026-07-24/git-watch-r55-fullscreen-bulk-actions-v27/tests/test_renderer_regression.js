...
assert(exportedHtml.includes(exportClone.outerHTML) && exportedHtml.includes('<meta name="git-watch-export-version" content="55">') && exportedHtml.includes('<meta name="git-watch-visual-baseline" content="55">'), "export must embed the exact current HTML snapshot, export format 54, and the Revision 55 visual baseline");
assert(exportedHtml.includes("<!-- GIT WATCH OFFLINE FILE GUIDE") && exportedHtml.includes("API-DEPENDENT SELECTION AND MUTATION CONTROLS OMITTED") && exportedHtml.includes("SUCCESS POPUP TOKENS (Revision 55)") && exportedHtml.includes("font: embedded Alumni Sans SC ExtraBold 800") && exportedHtml.includes("border radius: 25px") && exportedHtml.includes("minimum width: 589.7103px") && exportedHtml.includes("minimum height: 245.1429px") && exportedHtml.includes("padding: 83.5714px 70.3257px") && exportedHtml.includes("text size: 60.54px") && exportedHtml.includes("letter spacing: 0.04em") && exportedHtml.includes("background fill alpha: 95%") && exportedHtml.includes("dwell: 2000ms, then fade: 500ms") && exportedHtml.includes("press master gain: 0.4 → 1.2 (3x amplitude)") && exportedHtml.includes("inline sound: Cuelume 0.1.2 success and data-cuelume-press"), "export must explain offline scope and the final Revision 55 popup/font/opacity/timing/Cuelume tokens for readable future modification");
