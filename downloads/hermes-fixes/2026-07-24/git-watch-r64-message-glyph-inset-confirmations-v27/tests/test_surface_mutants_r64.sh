...
for kind in empty-directory broken-symlink; do
  ...
  set +e
  GIT_WATCH_R64_SKIP_SURFACE_MUTANTS=1 bash "$mutant/VERIFY.command" > "$TMP/$kind-verify.log" 2>&1
  verify_status=$?
  set -e
  [[ "$verify_status" != "0" ]]
  ! grep -q '^GIT_WATCH_R64_PACKAGE_VERIFICATION=PASS$' "$TMP/$kind-verify.log"
  ...
  set -e
  [[ "$install_status" != "0" ]]
  ! grep -q '^GIT_WATCH_R64_RUNTIME_INSTALL=PASS$' "$TMP/$kind-install.log"
  assert_home "$home" "$TMP/$kind-before"
done

echo 'GIT_WATCH_R64_FILESYSTEM_SURFACE_MUTANTS=PASS'
