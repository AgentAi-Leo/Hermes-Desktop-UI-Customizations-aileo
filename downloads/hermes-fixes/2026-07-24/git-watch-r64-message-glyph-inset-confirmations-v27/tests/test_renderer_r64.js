...
assert(source.includes('<meta name="git-watch-export-version" content="64">'), 'export schema must advance to Revision 64');
assert(source.includes('<meta name="git-watch-visual-baseline" content="64">'), 'visual baseline must identify Revision 64');
assert(source.includes('Export format 64 · Visual baseline 64'), 'visible export provenance must identify Revision 64');
...
assert(source.includes('CONFIRMATION DIALOG TOKENS (Revision 64)'), 'offline guide must document the rounded Alumni Sans SC confirmation dialog');
...
assert(source.includes('title: 68px; message: 44px; destructive message: 32px; action buttons: 76px minimum height / 24px labels'), 'export guide must document Revision 64 dialog scaling');
assert(source.includes('The message and adjacent actions share an intrinsic-width content grid; the action-group right edge aligns to the message right edge, with 28px panel padding preserved to the border.'), 'export guide must document actual message-edge alignment and right inset');
assert(source.includes('.git-comments-action-modal{width:max-content;max-width:min(869px,calc(100vw - 64px));max-height:calc(100vh - 64px);overflow:auto;box-sizing:border-box;display:grid;grid-template-columns:max-content;border:1px solid #475569;border-radius:25px;'), 'confirmation panel must remain content-fit, screenshot-bounded, and viewport-scrollable');
assert(source.includes('.git-comments-action-content{grid-column:1;display:grid;grid-template-columns:max-content;width:max-content;justify-self:start}'), 'message and adjacent actions must share their own intrinsic-width content grid');
assert(source.includes('.git-comments-action-buttons{grid-column:1;grid-row:2;justify-self:end;display:flex;align-items:center;gap:24px}'), 'confirmation button-group right edge must align to and never exceed the actual message-box right edge');
assert(source.includes('e("div", { className: "git-comments-action-content" },'), 'confirmation DOM must structurally group message and actions separately from the title');
assert(source.includes('padding:28px;font-family:"Alumni Sans SC",sans-serif'), 'confirmation panel must preserve a 28px inset between content and border');
for (const marker of [
...
]) assert(source.includes(marker), `existing semantic popup color marker missing: ${marker}`);
...
console.log('GIT_WATCH_R64_RENDERER_CONTRACT=PASS');
