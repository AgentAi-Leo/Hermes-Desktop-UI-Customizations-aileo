...
script:oldObfuscatedLink()">Obfuscated unsafe link</a>
  ...
  it("restores the exact top Portfolio Position Comparison, removes the producer bottom comparison, and keeps quote rows and CSV data", () => {
    ...
    expect(result).toContain("grid-template-columns: minmax(260px, 340px) max-content minmax(0, 1fr) !important");
  ...
  it("normalizes actual section-card stock archives into one horizontal quote row per ticker", () => {
    ...
    expect(result).toContain('<section class="hermes-stock-row" data-ticker="AAPL">');
    expect(result.match(/<h2 class="hermes-stock-today-title">TODAY'S PRICE<\/h2>/g)).toHaveLength(1);
    expect(result.indexOf(`<h2 class="hermes-stock-today-title">TODAY'S PRICE</h2>`)).toBeLessThan(
      result.indexOf('<section class="hermes-stock-row" data-ticker="AAPL">'),
    );
    expect(result).toContain('.hermes-stock-today-title { margin: 12px 0 18px !important; color: #eef3ff !important; font-family: inherit !important; font-size: 64px !important; font-weight: 700 !important; line-height: 1.05 !important; letter-spacing: .015em !important; }');
    expect(result).toContain('@media (max-width: 900px) { .hermes-stock-today-title { font-size: 48px !important; } }');
    expect(result).toContain('<span class="hermes-stock-ticker">AAPL</span>');
    ...
    expect(result).toContain('grid-template-columns: minmax(260px, 340px) max-content minmax(0, 1fr) !important; gap: 16px !important;');
    expect(result).toContain('.hermes-stock-row-date { grid-column: 2 !important;');
    expect(result).toContain('justify-self: start !important; padding: 9px 18px !important;');
    ...
    expect(result).toContain('@media (max-width: 1100px) { .hermes-stock-row { grid-template-columns: minmax(260px, 1fr) max-content !important;');
  ...
  it("adds one Stock header summary from the portfolio total without changing CSV bytes", () => {
    ...
    expect(result).toContain(".hermes-stock-summary-value { display: block !important; font-size: 56.96px !important; line-height: 1.05 !important; font-weight: 400 !important;");
    expect(result).toContain(".hermes-stock-summary-value { font-size: 42.72px !important; }");
