...
assert(source.includes('.git-comments-action-modal{width:max-content;max-width:calc(100vw - 64px);'), 'confirmation modal needs content-fit geometry bounded by the viewport');
...
assert(source.includes('<meta name="git-watch-export-version" content="62">'), 'export schema must advance to Revision 62');
assert(source.includes('<meta name="git-watch-visual-baseline" content="62">'), 'visual baseline must identify Revision 62');
assert(source.includes('Export format 62 · Visual baseline 62'), 'visible export provenance must identify Revision 62');
...
assert(source.includes('CONFIRMATION DIALOG TOKENS (Revision 62)'), 'offline guide must document the rounded Alumni Sans SC confirmation dialog');
...
assert(source.includes('title: 78px; message: 51px; action buttons: 76px minimum height / 24px labels'), 'export guide must document Revision 62 dialog scaling');
assert(source.includes('archived bulk colors: DESELECT ALL yellow; UNARCHIVE SELECTED green'), 'export guide must document Revision 62 semantic bulk colors');
assert(source.includes('.git-comments-action-modal{width:max-content;max-width:calc(100vw - 64px);max-height:calc(100vh - 64px);overflow:auto;box-sizing:border-box;display:grid;grid-template-columns:max-content max-content;column-gap:24px;border:1px solid #475569;border-radius:25px;'), 'confirmation panel must shrink to content, align buttons after the message column, and remain viewport-scrollable');
assert(source.includes('.git-comments-action-buttons{grid-column:2;grid-row:3;justify-self:start;display:flex;align-items:center;gap:24px}'), 'confirmation buttons must begin after the message column rather than at the modal edge');
...
console.log('GIT_WATCH_R62_RENDERER_CONTRACT=PASS');
