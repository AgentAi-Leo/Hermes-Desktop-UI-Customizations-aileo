...
HOST_PREDECESSOR_SHA = "b5ed1687d0692342e105384821e8e078977c3492c080c1ab273080986fcb6022"
...
def authorities() -> tuple[bytes, bytes, bytes, bytes]:
    ...
    if digest(host_predecessor) != HOST_PREDECESSOR_SHA:
        raise SystemExit("PINNED_DASHBOARD_API_HOST_PREDECESSOR_MISMATCH")
    if digest(hardened) != HARDENED_SHA:
        raise SystemExit("PINNED_DASHBOARD_API_HARDENED_MISMATCH")
    return legacy_predecessor, current_predecessor, host_predecessor, hardened
...
def main() -> int:
    ...
    legacy_predecessor, current_predecessor, host_predecessor, hardened = authorities()
    ...
    elif actual_sha == HOST_PREDECESSOR_SHA and actual == host_predecessor:
        atomic_write(target, hardened)
        if target.read_bytes() != hardened:
            print("BRIEFS_DASHBOARD_API_POST_WRITE_MISMATCH", file=sys.stderr)
            return 1
        print("BRIEFS_DASHBOARD_API=MIGRATED_FROM_PINNED_HOST_PRODUCTION")
