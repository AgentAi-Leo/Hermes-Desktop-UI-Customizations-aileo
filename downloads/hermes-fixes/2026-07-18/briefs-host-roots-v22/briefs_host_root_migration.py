...
def patch_env_loader(repo: Path, backup: Path) -> dict:
    ...
    marker = "class _SafeStatusStream:"
    changed = False
    if marker not in text:
        anchor = "_APPLIED_HOMES: set[str] = set()\n"
        if text.count(anchor) != 1:
            raise RuntimeError("env_loader.py safe-print insertion anchor did not match exactly once")
        helper = '''\n\nclass _SafeStatusStream:\n    \"\"\"Best-effort stream for cosmetic startup/status messages.\"\"\"\n\n    def write(self, text: str) -> int:\n        try:\n            return sys.stderr.write(text)\n        except (BrokenPipeError, OSError):\n            return len(text)\n\n    def flush(self) -> None:\n        try:\n            sys.stderr.flush()\n        except (BrokenPipeError, OSError):\n            pass\n\n\n_SAFE_STATUS_STREAM = _SafeStatusStream()\n'''
        text = text.replace(anchor, anchor + helper)
        changed = True
    replacements = text.count("file=sys.stderr")
    if replacements:
        text = text.replace("file=sys.stderr", "file=_SAFE_STATUS_STREAM")
        changed = True
    if "file=sys.stderr" in text:
        raise RuntimeError("env_loader.py still contains unsafe direct status writes")
    if "file=_SAFE_STATUS_STREAM" not in text:
        raise RuntimeError("env_loader.py has no safe status writes after patch")
    if changed:
        fd, temp_name = tempfile.mkstemp(prefix=".env_loader.", suffix=".py", dir=source.parent)
        try:
...
try:
    ...
    print("status probe", file=env_loader._SAFE_STATUS_STREAM)
finally:
    sys.stderr = old
...
'''
    ...
    return {
        "path": str(source),
        "changed": changed,
        "safe_write_occurrences": text.count("file=_SAFE_STATUS_STREAM"),
...
def apply(home: Path, repo: Path, profile_home: Path, package: Path, backup_root: Path) -> dict:
    ...
    try:
        for mode, spec in STREAMS.items():
            ...
            stage = Path(tempfile.mkdtemp(prefix=f".{spec['directory']}.v22-stage-", dir=zdownloads))
            try:
    ...
    rollback = backup_root / "ROLLBACK_BRIEFS_HOST_ROOTS_V22.command"
    rollback.write_text(
        "#!/usr/bin/env bash\nset -euo pipefail\n"
...
def rollback(backup_root: Path) -> None:
    ...
    print("ROLLBACK_BRIEFS_HOST_ROOTS_V22=PASS")
...
def main() -> int:
    ...
    print("BRIEFS_HOST_ROOTS_V22_MIGRATION=PASS")
    return 0
