...
assert(exportedHtml.includes(exportClone.outerHTML) && exportedHtml.includes('<meta name="git-watch-export-version" content="56">') && exportedHtml.includes('<meta name="git-watch-visual-baseline" content="56">'), "export must embed the exact current HTML snapshot and Revision 56 export/visual metadata");
assert(exportedHtml.includes("<!-- GIT WATCH OFFLINE FILE GUIDE") && exportedHtml.includes("API-DEPENDENT SELECTION AND MUTATION CONTROLS OMITTED") && exportedHtml.includes("SUCCESS POPUP TOKENS (Revision 56)") && exportedHtml.includes("CONFIRMATION DIALOG TOKENS (Revision 56)") && exportedHtml.includes("font: embedded Alumni Sans SC ExtraBold 800") && exportedHtml.includes("border radius: 25px") && exportedHtml.includes("minimum width: 589.7103px") && exportedHtml.includes("minimum height: 245.1429px") && exportedHtml.includes("padding: 83.5714px 70.3257px") && exportedHtml.includes("text size: 60.54px") && exportedHtml.includes("letter spacing: 0.04em") && exportedHtml.includes("background fill alpha: 95%") && exportedHtml.includes("dwell: 2000ms, then fade: 500ms") && exportedHtml.includes("press master gain: 0.4 → 1.2 (3x amplitude)") && exportedHtml.includes("inline sound: Cuelume 0.1.2 success and data-cuelume-press"), "export must explain offline scope and the final Revision 56 popup/dialog/font tokens for readable future modification");
...
assert(source.includes('font-family:"Alumni Sans SC",sans-serif') && source.includes('.git-comments-action-title{margin:0;color:#FFE6CB;font-size:26px;font-weight:800;letter-spacing:.04em}') && source.includes('.git-comments-action-message{margin:14px 0 24px;color:#cbd5e1;font-size:17px;line-height:1.55;font-weight:800;letter-spacing:.04em}') && source.includes('.git-comments-action-buttons .git-comments-button{font-family:"Alumni Sans SC",sans-serif;font-weight:800}'), "confirmation title, message, and buttons must use the embedded Alumni Sans SC ExtraBold Google font");
