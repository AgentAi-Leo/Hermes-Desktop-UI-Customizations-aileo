# Hermes Briefs deterministic templates v23

V23 is a cumulative update over V20. It preserves V22's summary, `TOTAL +/-`, color-coded `DAY +/-`, and exactly 25%-larger portfolio purchase dates. It also fixes Stock quote-row overflow by reducing the oversized ticker/company column, placing the yellow date pill immediately beside it, and moving all five statistics into fluid columns that remain inside the card. At narrower widths, the intact five-statistics group moves to a full-width second row rather than clipping. Calculations, sorting, totals, and both CSV contracts remain unchanged.
...
```bash
chmod +x INSTALL_BRIEFS_DETERMINISTIC_V23.command
./INSTALL_BRIEFS_DETERMINISTIC_V23.command
```
...
Use the backup-specific rollback path printed by the installer. `ROLLBACK_LATEST_BRIEFS_DETERMINISTIC_V23.command` is also provided for convenience.
