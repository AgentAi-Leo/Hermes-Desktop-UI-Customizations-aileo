    ...
    def test_apply_converts_links_retains_five_and_rollback_restores(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            original_env_loader = (
                ...
                "def _safe_status_print(message: str) -> None:\n"
                "    try:\n        print(message, file=sys.stderr)\n"
                "    except (BrokenPipeError, OSError):\n        pass\n"
                "def status():\n"
                "    _safe_status_print('one')\n    _safe_status_print('two')\n"
                "    _safe_status_print('three')\n    _safe_status_print('four')\n"
                "    _safe_status_print('five')\n    _safe_status_print('six')\n"
            )
            (repo / "hermes_cli" / "env_loader.py").write_text(original_env_loader, encoding="utf-8")
            backup = base / "backup"
            manifest = migration.apply(home, repo, profile, package, backup)
            self.assertEqual(manifest["version"], 23)
            for mode, stream in migration.STREAMS.items():
            ...
            self.assertEqual(env_text, original_env_loader)
            self.assertIn("def _safe_status_print", env_text)
            self.assertEqual(env_text.count("file=sys.stderr"), 1)
