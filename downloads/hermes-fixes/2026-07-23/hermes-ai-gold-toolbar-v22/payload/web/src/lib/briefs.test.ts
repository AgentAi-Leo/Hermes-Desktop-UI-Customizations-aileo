...
describe("brief helpers", () => {
  ...
  it("keeps the complete gold AI toolbar parent-owned through the oldest-to-latest wrap", () => {
    ...
    expect(briefsPage).toContain('briefDateNavigationWraps(listing.briefs, selected.date, direction)');
    expect(briefsPage).toContain('pendingAiViewportPositionRef.current = wraps ? null');
    expect(briefsPage).toContain('prepareBriefPreviewHtml(sourceHtml, kind, selected.generated_at, selected.date, isPersistentFullscreen && kind === "ai")');
