...
PACKAGE_NAME="git-watch-r62-content-fit-confirmations-v27"
...
restore_failure() {
  ...
  if [[ "$TOUCHED" == "1" ]]; then
    if ! (cd "$BACKUP" && shasum -a 256 -c BACKUP-CHECKSUMS.sha256 >/dev/null); then
      echo "GIT_WATCH_R62_RESTORE_FAILED=backup-integrity" >&2
      return 1
    fi
    ...
    if [[ "$failed" != "0" ]]; then
      echo "GIT_WATCH_R62_RESTORE_FAILED=copy" >&2
      return 1
    fi
    echo "GIT_WATCH_R62_RESTORED_AFTER_FAILURE=$BACKUP" >&2
  fi
}
...
required_renderer=[
    ...
    'event.key !== "Tab"',
    '<meta name="git-watch-export-version" content="62">',
    '<meta name="git-watch-visual-baseline" content="62">',
    'Export format 62 · Visual baseline 62',
    ...
    'CONFIRMATION DIALOG TOKENS (Revision 62)',
...
print('GIT_WATCH_R62_INSTALL_MARKERS=PASS')
PY
...
echo "GIT_WATCH_R62_BACKUP_RESTORE=PASS"
...
if [[ "${GIT_WATCH_R62_PAUSE_AFTER_RUNTIME:-0}" != "0" ]]; then
  [[ -z "${GIT_WATCH_R62_PAUSE_MARKER:-}" ]] || : > "$GIT_WATCH_R62_PAUSE_MARKER"
  sleep "$GIT_WATCH_R62_PAUSE_AFTER_RUNTIME"
fi

if [[ "${GIT_WATCH_R62_FAIL_AT:-}" == "after-runtime" ]]; then
...
TOUCHED=0
trap - EXIT
echo "GIT_WATCH_R62_RUNTIME_INSTALL=PASS"
