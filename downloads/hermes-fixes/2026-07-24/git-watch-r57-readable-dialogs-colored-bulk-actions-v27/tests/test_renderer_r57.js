...
assert(source.includes('<meta name="git-watch-export-version" content="57">'), 'export schema must advance to Revision 57');
assert(source.includes('<meta name="git-watch-visual-baseline" content="57">'), 'visual baseline must identify Revision 57');
assert(source.includes('Export format 57 · Visual baseline 57'), 'visible export provenance must identify Revision 57');
...
assert(source.includes('CONFIRMATION DIALOG TOKENS (Revision 57)'), 'offline guide must document the rounded Alumni Sans SC confirmation dialog');
...
assert(source.includes('title: 78px; message: 51px; action buttons: 76px minimum height / 24px labels'), 'export guide must document Revision 57 dialog scaling');
assert(source.includes('archived bulk colors: DESELECT ALL yellow; UNARCHIVE SELECTED green'), 'export guide must document Revision 57 semantic bulk colors');
assert(source.includes('.git-comments-action-modal{width:min(620px,calc(100vw - 64px));border:1px solid #475569;border-radius:25px;'), 'confirmation panel must use the same 25px rounded-rectangle radius as the success popup');
...
console.log('GIT_WATCH_R57_RENDERER_CONTRACT=PASS');
