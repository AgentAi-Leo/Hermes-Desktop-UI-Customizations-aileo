# Hermes Briefs deterministic templates v19

V19 is a cumulative update over V18. It keeps the verified deterministic Briefs implementation and changes only the large Stock header SUMMARY profit/loss value from bold (`850`) to normal (`400`) font weight while preserving its size, color, calculation, position, and CSV contracts.
...
```bash
chmod +x INSTALL_BRIEFS_DETERMINISTIC_V19.command
./INSTALL_BRIEFS_DETERMINISTIC_V19.command
```
...
Use the backup-specific rollback path printed by the installer. `ROLLBACK_LATEST_BRIEFS_DETERMINISTIC_V19.command` is also provided for convenience.
