# GIT WATCH Revision 63 — message-edge-aligned confirmation dialogs

This is the immutable successor to Revision 62 and does **not** rewrite that installed artifact, rejected Revision 61, Revision 60, rejected Revisions 57/58, published Revision 56, or the frozen Revision 52 Gold package.
...
- Replaces Revision 62's two-column message-plus-actions grid with one `max-content` column capped at `min(869px, calc(100vw - 64px))`.
- Places Cancel and the action button on row 3 of that same column with `justify-self: end`, so the button-group right edge equals—and never passes—the message right edge.
- Exact-runtime browser geometry at a 1920×1080 CSS viewport with DPR 2: Archive `672.89px`; Delete Watched `771.53px`; Unarchive Selected 1/2 `807.20px`/`830.97px`; Delete Selected 1/2 `844.48px`/`862.77px`; screenshot-C success reference `869.70px`. Every rendered action/message right-edge delta is `0` and every modal scroll width is at most `869px`.
...
- Includes behavioral mutants proving broken symlinks, empty directories, retired selectors, R62's second action column, and left-anchored action alignment fail closed without changing runtime or watchlist bytes.
...
- Advances standalone HTML export metadata, visible provenance, CSS parity documentation, and visual baseline to Revision 63. Exported files embed the corrected dashboard CSS byte-for-byte; API-dependent mutation/selection buttons remain intentionally omitted from read-only exports.
...
- `VERIFY.command` executes disposable install, data-preservation, injected-rollback, and checksum-valid retired-selector rejection tests through `tests/test_installer_r63.sh`.
