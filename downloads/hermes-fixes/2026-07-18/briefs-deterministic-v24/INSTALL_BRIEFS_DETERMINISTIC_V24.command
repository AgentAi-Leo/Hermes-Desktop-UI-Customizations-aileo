...
BACKUP="$HOME/.hermes/backups/briefs-deterministic-v24-$STAMP"
LOG="$HOME/.hermes/logs/briefs-deterministic-v24-${PORT}.log"
...
for marker in (b'v61-date-load-keyboard-focus',b'v74-ticker-above-company',b'hermes-stock-portfolio-summary',b'SUMMARY',b'v16-canonical-stock-wrapper-spacing',b'TOTAL +/-',b'DAY +/-',b'hermes-portfolio-day',b"TODAY'S PRICE",b'hermes-stock-today-title'):
...
if not re.search(rb'grid-template-columns:\s*minmax\(260px,\s*340px\)\s*max-content\s*minmax\(0,\s*1fr\)', data):
    raise SystemExit('Live bundle missing contained Stock quote-row columns')
...
if not re.search(rb'\.hermes-stock-today-title\s*\{[^}]*color:\s*#eef3ff\s*!important[^}]*font-family:\s*inherit\s*!important[^}]*font-size:\s*64px\s*!important', data):
    raise SystemExit("Live bundle missing plain white TODAY'S PRICE heading style")
if not re.search(rb'@media\s*\(max-width:\s*900px\)\s*\{\s*\.hermes-stock-today-title\s*\{\s*font-size:\s*48px\s*!important', data):
    raise SystemExit("Live bundle missing responsive TODAY'S PRICE heading size")
print('LIVE_BUNDLE_MARKERS=PASS')
PY
fi

cat > "$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V24.command" <<EOF
#!/usr/bin/env bash
set -euo pipefail
PREVIEW="${PREVIEW}"
...
echo "ROLLBACK_BRIEFS_DETERMINISTIC_V24=PASS"
EOF
chmod +x "$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V24.command"

trap - EXIT
echo "BRIEFS_DETERMINISTIC_V24_INSTALL=PASS"
...
echo "BACKUP=$BACKUP"
echo "ROLLBACK=$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V24.command"
echo "PREVIEW=http://127.0.0.1:$PORT/briefs-stocks?profile=$PROFILE&candidate=deterministic-v24"
