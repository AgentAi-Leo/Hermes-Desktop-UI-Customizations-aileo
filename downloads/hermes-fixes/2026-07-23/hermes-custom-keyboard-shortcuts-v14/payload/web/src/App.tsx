...
export default function App() {
  ...
  useEffect(() => {
    const handleThreeGoldKeyboardShortcut = (event: KeyboardEvent) => {
      ...
      const route = THREE_GOLD_KEYBOARD_ROUTES[event.key];
      if (route) {
      ...
      const cards = Array.from(
        document.querySelectorAll<HTMLElement>(GIT_WATCH_KEYBOARD_CARD_SELECTOR),
      ).filter((card) => {
      ...
      const activeCard =
        document.activeElement instanceof Element
          ? document.activeElement.closest<HTMLElement>(GIT_WATCH_KEYBOARD_CARD_SELECTOR)
          : null;
      const currentIndex = activeCard ? cards.indexOf(activeCard) : -1;
      const direction = event.key === "]" ? 1 : -1;
      const nextIndex = wrappedKeyboardIndex(currentIndex, cards.length, direction);
  ...
  }, [location.hash, location.search, navigate, normalizedPath]);
