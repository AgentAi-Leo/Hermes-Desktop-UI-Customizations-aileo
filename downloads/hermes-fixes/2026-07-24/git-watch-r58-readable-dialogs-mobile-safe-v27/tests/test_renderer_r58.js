...
assert(source.includes('<meta name="git-watch-export-version" content="58">'), 'export schema must advance to Revision 58');
assert(source.includes('<meta name="git-watch-visual-baseline" content="58">'), 'visual baseline must identify Revision 58');
assert(source.includes('Export format 58 · Visual baseline 58'), 'visible export provenance must identify Revision 58');
...
assert(source.includes('CONFIRMATION DIALOG TOKENS (Revision 58)'), 'offline guide must document the rounded Alumni Sans SC confirmation dialog');
...
assert(source.includes('title: 78px; message: 51px; action buttons: 76px minimum height / 24px labels'), 'export guide must document Revision 58 dialog scaling');
assert(source.includes('archived bulk colors: DESELECT ALL yellow; UNARCHIVE SELECTED green'), 'export guide must document Revision 58 semantic bulk colors');
...
console.log('GIT_WATCH_R58_RENDERER_CONTRACT=PASS');
