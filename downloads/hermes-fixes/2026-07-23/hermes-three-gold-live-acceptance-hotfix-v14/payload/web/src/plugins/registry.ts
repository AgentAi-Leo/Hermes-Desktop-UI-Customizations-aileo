...
/** Register a plugin component. Called synchronously by plugin JS bundles. */
export function registerPlugin(name: string, component: React.ComponentType) {
  const script =
    typeof document === "undefined"
      ? null
      : (document.currentScript as HTMLScriptElement | null);
  const rawGeneration = script?.getAttribute("data-hermes-plugin-generation");
  ...
  if (
    ...
    (scriptGeneration === null ||
      !Number.isSafeInteger(scriptGeneration) ||
      scriptGeneration !== _activeGeneration)
  ) {
    console.warn(
      `[plugins] Rejected stale or unscoped registration for ${name}`,
    );
    return;
  }
...
/** Clear all profile-scoped registrations and errors before switching profiles. */
...
/** Activate a new profile-load generation and invalidate every older script. */
export function beginPluginGeneration(generation: number) {
  if (!Number.isSafeInteger(generation) || generation < 1) {
    throw new Error("Plugin generation must be a positive safe integer");
  }
