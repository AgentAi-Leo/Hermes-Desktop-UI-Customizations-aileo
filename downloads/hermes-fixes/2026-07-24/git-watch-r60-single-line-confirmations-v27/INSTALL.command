...
PACKAGE_NAME="git-watch-r60-single-line-confirmations-v27"
...
restore_failure() {
  ...
  if [[ "$TOUCHED" == "1" ]]; then
    if ! (cd "$BACKUP" && shasum -a 256 -c BACKUP-CHECKSUMS.sha256 >/dev/null); then
      echo "GIT_WATCH_R60_RESTORE_FAILED=backup-integrity" >&2
      return 1
    fi
    ...
    if [[ "$failed" != "0" ]]; then
      echo "GIT_WATCH_R60_RESTORE_FAILED=copy" >&2
      return 1
    fi
    echo "GIT_WATCH_R60_RESTORED_AFTER_FAILURE=$BACKUP" >&2
  fi
}
...
required_renderer=[
    ...
    'event.key !== "Tab"',
    '<meta name="git-watch-export-version" content="60">',
    '<meta name="git-watch-visual-baseline" content="60">',
    'Export format 60 · Visual baseline 60',
    '.git-comments-action-modal{width:min(1440px,calc(100vw - 64px));max-height:calc(100vh - 64px);overflow:auto;box-sizing:border-box;border:1px solid #475569;border-radius:25px;',
    '@media(max-width:760px){.git-comments-archive-bulk-controls{align-items:stretch}.git-comments-archive-selected-count{width:100%}.git-comments-action-backdrop{padding:16px}.git-comments-action-modal{width:min(1440px,calc(100vw - 64px));padding:22px}',
    'font-family:"Alumni Sans SC",sans-serif',
    '.git-comments-action-title{margin:0;color:#FFE6CB;font-size:78px;font-weight:800;letter-spacing:.04em;white-space:nowrap}',
    '.git-comments-action-message{margin:14px 0 24px;color:#cbd5e1;font-size:51px;line-height:1.2;font-weight:800;letter-spacing:.04em;white-space:nowrap}',
    ...
    'CONFIRMATION DIALOG TOKENS (Revision 60)',
...
print('GIT_WATCH_R60_INSTALL_MARKERS=PASS')
PY
...
echo "GIT_WATCH_R60_BACKUP_RESTORE=PASS"
...
if [[ "${GIT_WATCH_R60_PAUSE_AFTER_RUNTIME:-0}" != "0" ]]; then
  [[ -z "${GIT_WATCH_R60_PAUSE_MARKER:-}" ]] || : > "$GIT_WATCH_R60_PAUSE_MARKER"
  sleep "$GIT_WATCH_R60_PAUSE_AFTER_RUNTIME"
fi

if [[ "${GIT_WATCH_R60_FAIL_AT:-}" == "after-runtime" ]]; then
...
TOUCHED=0
trap - EXIT
echo "GIT_WATCH_R60_RUNTIME_INSTALL=PASS"
