...
assert(
  ...
  "Revision 58 must increase confirmation-title text from 26px to 78px (200% increase)"
);
assert(
  source.includes(".git-comments-action-message{margin:14px 0 24px;color:#cbd5e1;font-size:51px"),
  "Revision 58 must increase confirmation-message text from 17px to 51px (200% increase)"
);
...
assert(
  ...
  "Revision 58 must double confirmation-button geometry and label size"
);
...
assert(
  ...
  "Revision 58 must render DESELECT ALL as yellow"
);
assert(
  ...
  "Revision 58 must render UNARCHIVE SELECTED as green"
);

assert(
  !source.includes(".git-comments-action-modal{width:min(620px,calc(100vw - 32px));padding:22px}"),
  "Revision 58 must not widen the accepted confirmation geometry at mobile widths"
);
assert(
  ...
  "Revision 58 mobile rule must preserve the exact accepted 64px viewport inset"
);
assert(
  ...
  "Revision 58 confirmation panel must remain reachable on short viewports"
);

console.log("GIT_WATCH_R58_DIALOG_TEXT_SCALE=PASS");
console.log("GIT_WATCH_R58_DIALOG_BUTTON_SCALE=PASS");
console.log("GIT_WATCH_R58_BULK_ACTION_COLORS=PASS");
console.log("GIT_WATCH_R58_MOBILE_GEOMETRY=PASS");
