# Hermes Briefs deterministic templates v21

V21 is a cumulative update over V20. It preserves the normal-weight, exactly 11%-smaller Stock header SUMMARY value and updates only the Portfolio Position Comparison presentation: the existing lifetime per-share `+/-` column is titled `TOTAL +/-`, and an adjacent `DAY +/-` column displays each ticker's canonical daily per-share dollar movement. Positive/negative colors, calculations, sorting, totals, and both CSV contracts remain unchanged.
...
```bash
chmod +x INSTALL_BRIEFS_DETERMINISTIC_V21.command
./INSTALL_BRIEFS_DETERMINISTIC_V21.command
```
...
Use the backup-specific rollback path printed by the installer. `ROLLBACK_LATEST_BRIEFS_DETERMINISTIC_V21.command` is also provided for convenience.
