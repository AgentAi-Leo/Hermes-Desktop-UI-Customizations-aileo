...
/* V25: archive-independent full-width Stock rows. */
...
.hermes-stock-today-title { margin: 12px 0 18px !important; color: #eef3ff !important; font-family: inherit !important; font-size: 64px !important; font-weight: 700 !important; line-height: 1.05 !important; letter-spacing: .015em !important; }
main.grid { display: flex !important; flex-direction: column !important; gap: 12px !important; }
.hermes-stock-row { grid-column: 1 / -1 !important; width: 100% !important; max-width: none !important; min-width: 0 !important; box-sizing: border-box !important; display: grid !important; grid-template-columns: minmax(260px, 340px) max-content minmax(0, 1fr) !important; gap: 16px !important; align-items: center !important; margin: 0 !important; padding: 20px 24px !important; border: 1px solid var(--line, #334155) !important; border-radius: 16px !important; background: var(--panel, #111827) !important; box-shadow: none !important; }
...
.hermes-stock-row-date { grid-column: 2 !important; display: inline-flex !important; min-height: 42px !important; align-items: center !important; justify-content: center !important; justify-self: start !important; padding: 9px 18px !important; border-radius: 999px !important; background: #ffd166 !important; color: #0b1020 !important; font-size: 1.05rem !important; font-weight: 850 !important; line-height: 1.15 !important; letter-spacing: .02em !important; text-align: center !important; white-space: nowrap !important; }
...
@media (max-width: 1200px) { .hermes-stock-company { font-size: 1.05rem !important; } .hermes-stock-row-date { padding: 8px 14px !important; font-size: .9rem !important; } .hermes-stock-metrics { gap: 8px !important; } }
@media (max-width: 900px) { .hermes-stock-today-title { font-size: 48px !important; } }
@media (max-width: 1100px) { .hermes-stock-row { grid-template-columns: minmax(260px, 1fr) max-content !important; gap: 14px 12px !important; } .hermes-stock-identity { grid-column: 1 !important; } .hermes-stock-row-date { grid-column: 2 !important; } .hermes-stock-metrics { grid-column: 1 / -1 !important; grid-row: 2 !important; width: 100% !important; } }
...
function normalizeStockQuoteRows(html: string): string {
  ...
  const withoutExistingTitle = normalizedHtml.replace(
    /<h2\b[^>]*\bclass\s*=\s*(?:"[^"]*\bhermes-stock-today-title\b[^"]*"|'[^']*\bhermes-stock-today-title\b[^']*')[^>]*>\s*TODAY'S PRICE\s*<\/h2\s*>/gi,
    "",
  );
  return withoutExistingTitle.replace(
    /<section\b(?=[^>]*\bclass\s*=\s*(?:"[^"]*\bhermes-stock-row\b[^"]*"|'[^']*\bhermes-stock-row\b[^']*'))/i,
    `<h2 class="hermes-stock-today-title">TODAY'S PRICE</h2><section`,
  );
}
