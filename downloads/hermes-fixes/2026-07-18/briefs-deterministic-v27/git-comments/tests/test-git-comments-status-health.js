...
assert(contextRows.every((row) => String(row.children[0]?.props?.className || "").startsWith("git-comments-comment-label ") && String(row.children[1]?.props?.className || "").startsWith("git-comments-current-state ") && String(row.children[2]?.props?.className || "") === "git-comments-status-text"), "COMMENTS must sit left of OPEN/CLOSED and its metadata text");
