...
def prune_ai(root: Path, keep: int = 5) -> None:
    dates = sorted({m.group(1) for p in root.iterdir() if p.is_file() and (m := re.fullmatch(r"AI_Brief_(20\d\d-\d\d-\d\d)\.(?:html|md)", p.name))}, reverse=True)
    for date in dates[keep:]:
        for suffix in ("html", "md"):
            (root / f"AI_Brief_{date}.{suffix}").unlink(missing_ok=True)
...
def publish(mode: str) -> int:
    ...
    if mode == "ai":
        ...
        root = AI_ROOT
        html_path = root / f"AI_Brief_{date}.html"
        md_path = root / f"AI_Brief_{date}.md"
