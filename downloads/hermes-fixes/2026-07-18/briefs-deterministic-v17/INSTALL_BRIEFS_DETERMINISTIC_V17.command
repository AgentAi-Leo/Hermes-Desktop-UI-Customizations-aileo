...
BACKUP="$HOME/.hermes/backups/briefs-deterministic-v17-$STAMP"
LOG="$HOME/.hermes/logs/briefs-deterministic-v17-${PORT}.log"
...
for marker in (b'v61-date-load-keyboard-focus',b'v74-ticker-above-company',b'utterance.rate=1.15',b'hermes-stock-portfolio-summary',b'SUMMARY',b'v16-canonical-stock-wrapper-spacing'):
...
cat > "$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V17.command" <<EOF
#!/usr/bin/env bash
set -euo pipefail
PREVIEW="${PREVIEW}"
...
echo "ROLLBACK_BRIEFS_DETERMINISTIC_V17=PASS"
EOF
chmod +x "$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V17.command"

trap - EXIT
echo "BRIEFS_DETERMINISTIC_V17_INSTALL=PASS"
...
echo "BACKUP=$BACKUP"
echo "ROLLBACK=$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V17.command"
echo "PREVIEW=http://127.0.0.1:$PORT/briefs-stocks?profile=$PROFILE&candidate=deterministic-v17"
