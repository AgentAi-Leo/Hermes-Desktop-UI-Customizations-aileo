...
@router.post("/watchlist/bulk-archived")
def bulk_archived_watch_urls(payload: dict[str, Any]) -> dict:
    action = str(payload.get("action") or "").strip().lower()
    raw_ids = payload.get("ids")
    if action not in {"restore", "delete"}:
        raise HTTPException(status_code=400, detail="Bulk archived action must be restore or delete")
    if not isinstance(raw_ids, list) or not raw_ids:
        raise HTTPException(status_code=400, detail="Select at least one archived watchlist entry")

    identifiers = [str(value or "").strip().lower() for value in raw_ids]
    if any(not identifier for identifier in identifiers):
        raise HTTPException(status_code=400, detail="Bulk archived IDs must be non-empty strings")
    if len(set(identifiers)) != len(identifiers):
        raise HTTPException(status_code=400, detail="Bulk archived IDs must be unique")

    watchlist = _watchlist()
    archived_by_id = {
        str(item.get("id") or "").lower(): item
        for item in watchlist["archived"]
    }
    missing = [identifier for identifier in identifiers if identifier not in archived_by_id]
    if missing:
        raise HTTPException(status_code=404, detail=f"Archived watchlist entries not found: {', '.join(missing)}")

    selected = set(identifiers)
    affected = [
        item for item in watchlist["archived"]
        if str(item.get("id") or "").lower() in selected
    ]
    watchlist["archived"] = [
        item for item in watchlist["archived"]
        if str(item.get("id") or "").lower() not in selected
    ]
    ...
    if action == "restore":
        ...
        for entry in affected:
            ...
            entry["restored_at"] = restored_at
        watchlist["active"].extend(affected)
