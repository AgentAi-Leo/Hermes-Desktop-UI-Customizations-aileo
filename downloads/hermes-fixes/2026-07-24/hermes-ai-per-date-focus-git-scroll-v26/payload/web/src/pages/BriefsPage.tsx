...
export default function BriefsPage({ kind, title, emptyMessage }: BriefsPageProps) {
  ...
  const activeAiPreviewDateRef = useRef<string | null>(null);
  ...
  const rememberAiViewportForDateChange = (
    targetDate: string,
    _direction?: "newer" | "older",
  ) => {
    if (kind !== "ai") return;
    pendingAiViewportPositionRef.current = aiViewportByDateRef.current.get(targetDate)
      ?? { scrollX: 0, scrollY: 0 };
  };
  ...
  useEffect(() => {
    ...
    void api
      ...
      .then(async (blob) => {
        ...
        const html = prepareBriefPreviewHtml(sourceHtml, kind, selected.generated_at, selected.date, kind === "ai");
        if (!active) return;
        if (kind === "stock") {
  ...
  useEffect(() => {
    ...
    const handleDateNavigation = (event: MessageEvent) => {
      ...
      if (event.data?.type === "hermes-ai-viewport-state") {
        const activeDate = activeAiPreviewDateRef.current;
        ...
        if (
          ...
          aiViewportByDateRef.current.set(activeDate, {
            scrollX: Math.max(0, position.scrollX),
            scrollY: Math.max(0, position.scrollY),
            topicIndex: Math.trunc(position.topicIndex),
          });
        }
        return;
      }
      ...
      if (kind === "ai") {
        ...
        rememberAiViewportForDateChange(target.date, event.data.direction);
      } else if (
        kind === "stock" &&
        Number.isFinite(position?.scrollX) &&
        Number.isFinite(position?.scrollY)
              ...
              {previewUrl && (
                <iframe
                  ...
                  onLoad={(event) => {
                    ...
                    if (kind === "ai") {
                      activeAiPreviewDateRef.current = selected?.date ?? null;
