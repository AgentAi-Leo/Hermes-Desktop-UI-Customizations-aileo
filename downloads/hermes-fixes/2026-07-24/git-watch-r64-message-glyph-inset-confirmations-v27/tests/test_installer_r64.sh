...
grep -q '^GIT_WATCH_R64_RUNTIME_INSTALL=PASS$' "$TMP/success.log"
...
set +e
HERMES_HOME="$ROLLBACK_ROOT" HERMES_PROFILE=local-ai-assist1 HERMES_PYTHON="$PYTHON_BIN" GIT_WATCH_R64_FAIL_AT=after-runtime \
  bash "$PACKAGE_DIR/INSTALL.command" > "$TMP/rollback.log" 2>&1
ROLLBACK_STATUS=$?
set -e
[[ "$ROLLBACK_STATUS" == "91" ]]
grep -q '^GIT_WATCH_R64_RESTORED_AFTER_FAILURE=' "$TMP/rollback.log"
! grep -q '^GIT_WATCH_R64_RUNTIME_INSTALL=PASS$' "$TMP/rollback.log"
...
set -e
[[ "$REJECT_STATUS" != "0" ]]
! grep -q '^GIT_WATCH_R64_INSTALL_MARKERS=PASS$' "$TMP/reject.log"
! grep -q '^GIT_WATCH_R64_RUNTIME_INSTALL=PASS$' "$TMP/reject.log"
assert_snapshot "$REJECT_ROOT" "$TMP/reject-before"

# A checksum-valid left-anchored button regression must also fail closed.
ALIGNMENT_MUTANT_PARENT="$TMP/alignment-mutant-parent"
mkdir -p "$ALIGNMENT_MUTANT_PARENT"
cp -a "$PACKAGE_DIR" "$ALIGNMENT_MUTANT_PARENT/package"
"$PYTHON_BIN" - "$ALIGNMENT_MUTANT_PARENT/package" <<'PY'
from pathlib import Path
import hashlib, sys
root = Path(sys.argv[1])
renderer = root / 'payload/dashboard/dist/index.js'
text = renderer.read_text(encoding='utf-8')
old = '.git-comments-action-buttons{grid-column:1;grid-row:2;justify-self:end;display:flex;align-items:center;gap:24px}'
new = '.git-comments-action-buttons{grid-column:1;grid-row:2;justify-self:start;display:flex;align-items:center;gap:24px}'
...
(root / 'CHECKSUMS.sha256').write_text('\n'.join(lines) + '\n', encoding='utf-8')
PY
ALIGNMENT_REJECT_ROOT="$TMP/alignment-reject-home"
make_home "$ALIGNMENT_REJECT_ROOT"
snapshot_runtime "$ALIGNMENT_REJECT_ROOT" "$TMP/alignment-reject-before"
set +e
HERMES_HOME="$ALIGNMENT_REJECT_ROOT" HERMES_PROFILE=local-ai-assist1 HERMES_PYTHON="$PYTHON_BIN" \
  bash "$ALIGNMENT_MUTANT_PARENT/package/INSTALL.command" > "$TMP/alignment-reject.log" 2>&1
ALIGNMENT_REJECT_STATUS=$?
set -e
[[ "$ALIGNMENT_REJECT_STATUS" != "0" ]]
! grep -q '^GIT_WATCH_R64_INSTALL_MARKERS=PASS$' "$TMP/alignment-reject.log"
! grep -q '^GIT_WATCH_R64_RUNTIME_INSTALL=PASS$' "$TMP/alignment-reject.log"
assert_snapshot "$ALIGNMENT_REJECT_ROOT" "$TMP/alignment-reject-before"

# A checksum-valid zero-padding regression must fail before any runtime mutation.
INSET_MUTANT_PARENT="$TMP/inset-mutant-parent"
mkdir -p "$INSET_MUTANT_PARENT"
cp -a "$PACKAGE_DIR" "$INSET_MUTANT_PARENT/package"
"$PYTHON_BIN" - "$INSET_MUTANT_PARENT/package" <<'PY'
from pathlib import Path
import hashlib, sys
root = Path(sys.argv[1])
renderer = root / 'payload/dashboard/dist/index.js'
text = renderer.read_text(encoding='utf-8')
old = 'padding:28px;font-family:"Alumni Sans SC",sans-serif'
new = 'padding:0;font-family:"Alumni Sans SC",sans-serif'
assert text.count(old) == 1
renderer.write_text(text.replace(old, new), encoding='utf-8')
lines = []
for path in sorted((p for p in root.rglob('*') if p.is_file() and p.name != 'CHECKSUMS.sha256'), key=lambda p: p.relative_to(root).as_posix()):
...
(root / 'CHECKSUMS.sha256').write_text('\n'.join(lines) + '\n', encoding='utf-8')
PY
INSET_REJECT_ROOT="$TMP/inset-reject-home"
make_home "$INSET_REJECT_ROOT"
snapshot_runtime "$INSET_REJECT_ROOT" "$TMP/inset-reject-before"
set +e
HERMES_HOME="$INSET_REJECT_ROOT" HERMES_PROFILE=local-ai-assist1 HERMES_PYTHON="$PYTHON_BIN" \
  bash "$INSET_MUTANT_PARENT/package/INSTALL.command" > "$TMP/inset-reject.log" 2>&1
INSET_REJECT_STATUS=$?
set -e
[[ "$INSET_REJECT_STATUS" != "0" ]]
! grep -q '^GIT_WATCH_R64_INSTALL_MARKERS=PASS$' "$TMP/inset-reject.log"
! grep -q '^GIT_WATCH_R64_RUNTIME_INSTALL=PASS$' "$TMP/inset-reject.log"
assert_snapshot "$INSET_REJECT_ROOT" "$TMP/inset-reject-before"

echo 'GIT_WATCH_R64_INSTALLER_LIFECYCLE=PASS'
