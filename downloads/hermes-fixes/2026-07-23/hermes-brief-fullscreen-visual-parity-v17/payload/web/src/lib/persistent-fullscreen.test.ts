...
describe("persistent route fullscreen", () => {
  ...
  it("preserves the old content-only Brief presentation inside the persistent fullscreen shell", () => {
    const app = readFileSync(new URL("../App.tsx", import.meta.url), "utf8");
    const briefsPage = readFileSync(new URL("../pages/BriefsPage.tsx", import.meta.url), "utf8");
    ...
    expect(briefsPage).toContain('isPersistentFullscreen ? "h-dvh gap-0 overflow-hidden"');
