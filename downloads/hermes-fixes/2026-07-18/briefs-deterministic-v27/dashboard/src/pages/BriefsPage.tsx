...
export default function BriefsPage({ kind, title, emptyMessage }: BriefsPageProps) {
  ...
  const rememberAiViewportForDateChange = (targetDate: string) => {
    ...
    pendingAiViewportPositionRef.current = aiViewportByDateRef.current.get(targetDate) ?? position;
  };
  ...
  useEffect(() => {
    ...
    const handlePreviewFullscreenShortcut = (event: KeyboardEvent) => {
      ...
      void (async () => {
        try {
          ...
          // Browser fullscreen denial leaves the embedded preview unchanged.
        }
      })();
    };

    window.addEventListener("keydown", handlePreviewFullscreenShortcut, true);
    return () => window.removeEventListener("keydown", handlePreviewFullscreenShortcut, true);
  }, [previewUrl]);
  ...
  useEffect(() => {
    ...
    const handleDateNavigation = (event: MessageEvent) => {
      ...
      if (kind === "ai" && Number.isInteger(position?.topicIndex)) {
        ...
        pendingAiViewportPositionRef.current = aiViewportByDateRef.current.get(target.date) ?? position;
      } else if (
        kind === "stock" &&
        Number.isFinite(position?.scrollX) &&
        Number.isFinite(position?.scrollY)
  ...
  useEffect(() => {
    if (kind !== "ai" && kind !== "stock") return;
    const fullscreenMessagePrefix = kind === "ai" ? "hermes-ai" : "hermes-stock";

    const publishFullscreenState = () => {
      const iframe = previewRef.current;
      iframe?.contentWindow?.postMessage(
        { type: `${fullscreenMessagePrefix}-fullscreen-state`, active: document.fullscreenElement === iframe },
        "*",
      );
    };
    const handleBriefFullscreen = async (event: MessageEvent) => {
      const iframe = previewRef.current;
      if (!iframe || event.source !== iframe.contentWindow) return;
      if (event.data?.type !== `${fullscreenMessagePrefix}-fullscreen-toggle`) return;
      try {
        if (document.fullscreenElement === iframe) {
          await document.exitFullscreen();
        } else {
    ...
    window.addEventListener("message", handleBriefFullscreen);
    document.addEventListener("fullscreenchange", publishFullscreenState);
    return () => {
      window.removeEventListener("message", handleBriefFullscreen);
      document.removeEventListener("fullscreenchange", publishFullscreenState);
    };
  }, [kind]);
          ...
          <CardContent className="flex h-full min-h-0 flex-col p-0">
            ...
            >
              ...
              {kind === "stock" && (
                ...
                >
                  ...
                  <span className="inline-flex min-h-[38px] items-center whitespace-nowrap text-[30px] font-normal leading-none tracking-[0.04em] sm:min-h-[42px] sm:text-[42px] sm:leading-[42px]">
                    DATE
                  </span>
                  <button
                    type="button"
            ...
            <div className="relative min-h-0 flex-1" style={{ backgroundColor: previewCanvasColor }}>
              ...
              {kind === "ai" && !previewLoading && previewUrl && selected && (
                <div className="absolute inset-x-2 top-2 z-20 max-w-[calc(100%-1rem)] rounded-md bg-[#0b1020]/95 p-1.5 shadow-xl sm:left-auto sm:right-4 sm:top-3">
