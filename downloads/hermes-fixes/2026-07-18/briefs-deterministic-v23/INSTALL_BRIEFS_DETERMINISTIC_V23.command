...
BACKUP="$HOME/.hermes/backups/briefs-deterministic-v23-$STAMP"
LOG="$HOME/.hermes/logs/briefs-deterministic-v23-${PORT}.log"
...
cat > "$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V23.command" <<EOF
#!/usr/bin/env bash
set -euo pipefail
PREVIEW="${PREVIEW}"
...
echo "ROLLBACK_BRIEFS_DETERMINISTIC_V23=PASS"
EOF
chmod +x "$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V23.command"

trap - EXIT
echo "BRIEFS_DETERMINISTIC_V23_INSTALL=PASS"
...
echo "BACKUP=$BACKUP"
echo "ROLLBACK=$BACKUP/ROLLBACK_BRIEFS_DETERMINISTIC_V23.command"
echo "PREVIEW=http://127.0.0.1:$PORT/briefs-stocks?profile=$PROFILE&candidate=deterministic-v23"
