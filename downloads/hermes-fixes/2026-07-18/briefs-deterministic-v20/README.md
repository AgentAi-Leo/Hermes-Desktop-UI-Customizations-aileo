# Hermes Briefs deterministic templates v20

V20 is a cumulative update over V18. It keeps the verified deterministic Briefs implementation and changes only the large Stock header SUMMARY profit/loss value from bold (`850`) to normal (`400`) font weight while reducing its size by exactly 11%: desktop `64px` → `56.96px` and responsive `48px` → `42.72px`. Its color, calculation, position, and CSV contracts remain unchanged.
...
```bash
chmod +x INSTALL_BRIEFS_DETERMINISTIC_V20.command
./INSTALL_BRIEFS_DETERMINISTIC_V20.command
```
...
Use the backup-specific rollback path printed by the installer. `ROLLBACK_LATEST_BRIEFS_DETERMINISTIC_V20.command` is also provided for convenience.
