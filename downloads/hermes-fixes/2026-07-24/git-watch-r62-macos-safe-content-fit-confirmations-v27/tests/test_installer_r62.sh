...
grep -q '^GIT_WATCH_R62_RUNTIME_INSTALL=PASS$' "$TMP/success.log"
...
set +e
HERMES_HOME="$ROLLBACK_ROOT" HERMES_PROFILE=local-ai-assist1 HERMES_PYTHON="$PYTHON_BIN" GIT_WATCH_R62_FAIL_AT=after-runtime \
  bash "$PACKAGE_DIR/INSTALL.command" > "$TMP/rollback.log" 2>&1
ROLLBACK_STATUS=$?
set -e
[[ "$ROLLBACK_STATUS" == "91" ]]
grep -q '^GIT_WATCH_R62_RESTORED_AFTER_FAILURE=' "$TMP/rollback.log"
! grep -q '^GIT_WATCH_R62_RUNTIME_INSTALL=PASS$' "$TMP/rollback.log"
...
set -e
[[ "$REJECT_STATUS" != "0" ]]
! grep -q '^GIT_WATCH_R62_INSTALL_MARKERS=PASS$' "$TMP/reject.log"
! grep -q '^GIT_WATCH_R62_RUNTIME_INSTALL=PASS$' "$TMP/reject.log"
assert_snapshot "$REJECT_ROOT" "$TMP/reject-before"

# A checksum-valid far-edge button regression must also fail closed.
...
old = '.git-comments-action-buttons{grid-column:2;grid-row:3;justify-self:start;display:flex;align-items:center;gap:24px}'
new = '.git-comments-action-buttons{display:flex;align-items:center;justify-content:flex-end;gap:24px}'
...
! grep -q '^GIT_WATCH_R62_INSTALL_MARKERS=PASS$' "$TMP/alignment-reject.log"
! grep -q '^GIT_WATCH_R62_RUNTIME_INSTALL=PASS$' "$TMP/alignment-reject.log"
assert_snapshot "$ALIGNMENT_REJECT_ROOT" "$TMP/alignment-reject-before"

echo 'GIT_WATCH_R62_INSTALLER_LIFECYCLE=PASS'
