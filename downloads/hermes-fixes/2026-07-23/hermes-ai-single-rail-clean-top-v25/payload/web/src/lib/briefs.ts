  ...
  function init() {
    ...
    document.addEventListener("keydown", (event) => {
      if (/^[1-9]$/.test(event.key)) {
        if (window.parent === window || event.metaKey || event.ctrlKey || event.altKey || event.shiftKey || textEditingTarget(event.target)) return;
        event.preventDefault();
        event.stopImmediatePropagation();
        window.parent.postMessage({ type: "hermes-custom-tab-shortcut", key: event.key }, "*");
        return;
      }
      if (event.key === "Enter") {
        event.preventDefault();
    ...
    window.addEventListener("message", (event) => {
      ...
      if (event.data?.type === "hermes-ai-playback-rate-restore") {
        setPlaybackRate(event.data.playbackRate);
        return;
      }
  ...
  document.addEventListener("keydown", (event) => {
    if (/^[1-9]$/.test(event.key)) {
      if (
        ...
        (event.target instanceof Element && event.target.closest('[contenteditable="true"], [role="textbox"]'))
      ) return;
      event.preventDefault();
      event.stopImmediatePropagation();
      window.parent.postMessage({ type: "hermes-custom-tab-shortcut", key: event.key }, "*");
...
function extractStockPrices(html: string): Map<PortfolioLot["ticker"], number> {
  ...
  for (const match of html.matchAll(cardSection)) {
    if (/\bhermes-stock-row\b/i.test(match[1])) {
      const ticker = match[1].match(/\bdata-ticker\s*=\s*["'](AAPL|AMZN|NVDA|SNAP|GOOGL|MSFT|DIS)["']/i)?.[1]
        ?? match[2].match(/<span\b[^>]*class\s*=\s*(?:"[^"]*\bhermes-stock-ticker\b[^"]*"|'[^']*\bhermes-stock-ticker\b[^']*')[^>]*>\s*(AAPL|AMZN|NVDA|SNAP|GOOGL|MSFT|DIS)\s*<\/span\s*>/i)?.[1];
      const price = match[2].match(/<(?:strong|span|div)\b[^>]*class\s*=\s*(?:"[^"]*\bhermes-stock-price\b[^"]*"|'[^']*\bhermes-stock-price\b[^']*')[^>]*>\s*(\$\s*[0-9][0-9,]*(?:\.\d+)?)\s*<\/(?:strong|span|div)\s*>/i)?.[1];
      if (ticker && price) candidates.push(`${ticker} ${price}`);
    } else if (/\bcard\b/i.test(match[1]) && !/<article\b/i.test(match[2])) {
      candidates.push(normalizeText(match[2]));
    }
  }
...
function canonicalAiDocument(html: string, selectedDate?: string): string {
  ...
  const playerDate = escape(date || "Date unavailable");
  const playerRail = `<div class="hermes-brief-player-placeholder" style="height: 66px;"></div><nav id="hermes-brief-player" class="hermes-brief-player-sticky" aria-label="Brief playback controls" tabindex="-1"><div class="hermes-player-shell"><button id="tts-prev" type="button" aria-label="Previous topic" title="Previous topic">←</button><button id="tts-toggle" type="button" aria-label="Play reading" aria-pressed="false" title="Play or pause">▶</button><button id="tts-next" type="button" aria-label="Next topic" title="Next topic">→</button><label class="hermes-player-volume"><input id="tts-volume" type="range" min="0" max="1" step="0.05" value="0.45" aria-label="Volume" title="Volume 45%"></label><button id="tts-speed" type="button" aria-label="Playback speed 1.25x" title="Playback speed">1.25x</button><span class="hermes-date-nav" role="group" aria-label="Cron date navigation"><button id="hermes-date-newer" type="button" aria-label="Load newer cron date" title="Newer cron date">‹</button><span class="hermes-date-nav-label">${playerDate}</span><button id="hermes-date-older" type="button" aria-label="Load older cron date" title="Older cron date">›</button></span></div></nav>`;
  return `<!doctype html><html lang="en" data-hermes-canonical-ai="v26"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI Morning Brief</title>${V25_AI_BASE_STYLE}</head><body>${playerRail}<main id="hermes-ai-canonical">${header}${takeaways}${topics}</main></body></html>`;
}
 ...
 */
export function prepareBriefPreviewHtml(
  html: string,
  kind?: BriefKind,
  generatedAt?: number,
  selectedDate?: string,
  parentOwnsAiToolbar = false,
): string {
  ...
  const withArchiveStyle = kind === "ai"
    ...
    : kind === "stock"
      ...
      : withPlayerStyle;
  const withToolbarOwnership = kind === "ai" && parentOwnsAiToolbar
    ? injectBeforeClosingTag(
        withArchiveStyle,
        "head",
        '<style id="hermes-ai-parent-toolbar-ownership">#hermes-brief-player,.hermes-brief-player-placeholder{display:none!important}</style>',
      )
    : withArchiveStyle;
  return kind === "stock"
    ? injectBeforeClosingTag(withToolbarOwnership, "body", `${STOCK_INTERACTION_CONTROLLER}${STOCK_SECTION_NAVIGATION_CONTROLLER}`)
    : injectBeforeClosingTag(withToolbarOwnership, "body", PLAYER_CONTROLLER);
}
...
export function briefDateNavigationWraps(
  briefs: BriefEntry[],
  currentDate: string,
  direction: "newer" | "older",
): boolean {
  if (briefs.length < 2) return false;
  const currentIndex = briefs.findIndex((brief) => brief.date === currentDate);
  if (currentIndex < 0) return false;
  return direction === "older" ? currentIndex === briefs.length - 1 : currentIndex === 0;
}
