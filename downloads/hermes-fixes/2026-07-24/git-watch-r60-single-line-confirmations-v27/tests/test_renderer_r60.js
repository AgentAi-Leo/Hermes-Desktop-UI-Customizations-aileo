...
assert(source.includes('<meta name="git-watch-export-version" content="60">'), 'export schema must advance to Revision 60');
assert(source.includes('<meta name="git-watch-visual-baseline" content="60">'), 'visual baseline must identify Revision 60');
assert(source.includes('Export format 60 · Visual baseline 60'), 'visible export provenance must identify Revision 60');
...
assert(source.includes('CONFIRMATION DIALOG TOKENS (Revision 60)'), 'offline guide must document the rounded Alumni Sans SC confirmation dialog');
...
assert(source.includes('title: 78px; message: 51px; action buttons: 76px minimum height / 24px labels'), 'export guide must document Revision 60 dialog scaling');
assert(source.includes('archived bulk colors: DESELECT ALL yellow; UNARCHIVE SELECTED green'), 'export guide must document Revision 60 semantic bulk colors');
assert(source.includes('.git-comments-action-modal{width:min(1440px,calc(100vw - 64px));max-height:calc(100vh - 64px);overflow:auto;box-sizing:border-box;border:1px solid #475569;border-radius:25px;'), 'confirmation panel must preserve exact width/radius and remain viewport-scrollable');
...
console.log('GIT_WATCH_R60_RENDERER_CONTRACT=PASS');
