...
PACKAGE_NAME="git-watch-r54-fullscreen-bulk-actions-v27"
...
restore_failure() {
  ...
  if [[ "$TOUCHED" == "1" ]]; then
    if ! (cd "$BACKUP" && shasum -a 256 -c BACKUP-CHECKSUMS.sha256 >/dev/null); then
      echo "GIT_WATCH_R54_RESTORE_FAILED=backup-integrity" >&2
      return 1
    fi
    ...
    if [[ "$failed" != "0" ]]; then
      echo "GIT_WATCH_R54_RESTORE_FAILED=copy" >&2
      return 1
    fi
    echo "GIT_WATCH_R54_RESTORED_AFTER_FAILURE=$BACKUP" >&2
  fi
}
...
print('GIT_WATCH_R54_LEDGER_COVERAGE=PASS')
PY
...
required_renderer=[
    ...
    'event.key !== "Tab"',
    '<meta name="git-watch-export-version" content="54">',
...
print('GIT_WATCH_R54_INSTALL_MARKERS=PASS')
PY
...
echo "GIT_WATCH_R54_BACKUP_RESTORE=PASS"
...
if [[ "${GIT_WATCH_R54_PAUSE_AFTER_RUNTIME:-0}" != "0" ]]; then
  [[ -z "${GIT_WATCH_R54_PAUSE_MARKER:-}" ]] || : > "$GIT_WATCH_R54_PAUSE_MARKER"
  sleep "$GIT_WATCH_R54_PAUSE_AFTER_RUNTIME"
fi

if [[ "${GIT_WATCH_R54_FAIL_AT:-}" == "after-runtime" ]]; then
...
TOUCHED=0
trap - EXIT
echo "GIT_WATCH_R54_RUNTIME_INSTALL=PASS"
