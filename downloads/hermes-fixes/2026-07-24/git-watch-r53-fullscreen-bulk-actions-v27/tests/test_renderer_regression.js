...
const exportClone = {
  ...
  querySelectorAll(selector) {
    assert.strictEqual(selector, ".git-comments-panel-add,.git-comments-success,.git-comments-error,.git-comments-button.add-toggle,.git-comments-button.archive,.git-comments-button.delete,.git-comments-button.restore,.git-comments-button.unarchive,.git-comments-button.view-archived,.git-comments-button.retry-connection,.git-comments-button.export-html,.git-comments-archive-bulk-controls,.git-comments-archive-select,.git-comments-action-backdrop");
...
assert(exportedHtml.includes(exportClone.outerHTML) && exportedHtml.includes('<meta name="git-watch-export-version" content="53">') && exportedHtml.includes('<meta name="git-watch-visual-baseline" content="53">'), "export must embed the exact current HTML snapshot, export format 53, and the Revision 53 visual baseline");
assert(exportedHtml.includes("<!-- GIT WATCH OFFLINE FILE GUIDE") && exportedHtml.includes("API-DEPENDENT SELECTION AND MUTATION CONTROLS OMITTED") && exportedHtml.includes("SUCCESS POPUP TOKENS (Revision 53)") && exportedHtml.includes("font: embedded Alumni Sans SC ExtraBold 800") && exportedHtml.includes("border radius: 25px") && exportedHtml.includes("minimum width: 589.7103px") && exportedHtml.includes("minimum height: 245.1429px") && exportedHtml.includes("padding: 83.5714px 70.3257px") && exportedHtml.includes("text size: 60.54px") && exportedHtml.includes("letter spacing: 0.04em") && exportedHtml.includes("background fill alpha: 95%") && exportedHtml.includes("dwell: 2000ms, then fade: 500ms") && exportedHtml.includes("press master gain: 0.4 → 1.2 (3x amplitude)") && exportedHtml.includes("inline sound: Cuelume 0.1.2 success and data-cuelume-press"), "export must explain offline scope and the final Revision 53 popup/font/opacity/timing/Cuelume tokens for readable future modification");
...
const archivedActions = archivedRows[0].children[2];
...
assert(source.match(/showSuccess\("SUCCESSFULLY DELETED!", 2000, "red"\)/g)?.length === 1, "active and archived single-delete actions must share one red success path");
...
assert(source.includes("Permanently delete this archived URL?") && source.includes("function ActionConfirmModal"), "archived DELETE action must require the fullscreen-safe in-app confirmation");
