...
grep -q '^GIT_WATCH_R57_RUNTIME_INSTALL=PASS$' "$TMP/success.log"
...
set +e
HERMES_HOME="$ROLLBACK_ROOT" HERMES_PROFILE=local-ai-assist1 HERMES_PYTHON="$PYTHON_BIN" GIT_WATCH_R57_FAIL_AT=after-runtime \
  bash "$PACKAGE_DIR/INSTALL.command" > "$TMP/rollback.log" 2>&1
ROLLBACK_STATUS=$?
set -e
[[ "$ROLLBACK_STATUS" == "91" ]]
grep -q '^GIT_WATCH_R57_RESTORED_AFTER_FAILURE=' "$TMP/rollback.log"
! grep -q '^GIT_WATCH_R57_RUNTIME_INSTALL=PASS$' "$TMP/rollback.log"
...
set -e
[[ "$REJECT_STATUS" != "0" ]]
! grep -q '^GIT_WATCH_R57_INSTALL_MARKERS=PASS$' "$TMP/reject.log"
! grep -q '^GIT_WATCH_R57_RUNTIME_INSTALL=PASS$' "$TMP/reject.log"
assert_snapshot "$REJECT_ROOT" "$TMP/reject-before"

echo 'GIT_WATCH_R57_INSTALLER_LIFECYCLE=PASS'
