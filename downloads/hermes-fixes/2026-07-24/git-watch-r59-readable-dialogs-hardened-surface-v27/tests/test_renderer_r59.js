...
assert(source.includes('<meta name="git-watch-export-version" content="59">'), 'export schema must advance to Revision 59');
assert(source.includes('<meta name="git-watch-visual-baseline" content="59">'), 'visual baseline must identify Revision 59');
assert(source.includes('Export format 59 · Visual baseline 59'), 'visible export provenance must identify Revision 59');
...
assert(source.includes('CONFIRMATION DIALOG TOKENS (Revision 59)'), 'offline guide must document the rounded Alumni Sans SC confirmation dialog');
...
assert(source.includes('title: 78px; message: 51px; action buttons: 76px minimum height / 24px labels'), 'export guide must document Revision 59 dialog scaling');
assert(source.includes('archived bulk colors: DESELECT ALL yellow; UNARCHIVE SELECTED green'), 'export guide must document Revision 59 semantic bulk colors');
assert(source.includes('.git-comments-action-modal{width:min(620px,calc(100vw - 64px));max-height:calc(100vh - 64px);overflow:auto;box-sizing:border-box;border:1px solid #475569;border-radius:25px;'), 'confirmation panel must preserve exact width/radius and remain viewport-scrollable');
...
console.log('GIT_WATCH_R59_RENDERER_CONTRACT=PASS');
