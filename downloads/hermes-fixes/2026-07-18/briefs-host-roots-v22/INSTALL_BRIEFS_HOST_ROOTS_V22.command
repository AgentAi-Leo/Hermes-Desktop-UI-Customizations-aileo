...
BACKUP="$HOME/.hermes/backups/briefs-host-roots-v22-$STAMP"
...
restore_after_failure() {
  ...
  echo "BRIEFS_HOST_ROOTS_V22_INSTALL=FAILED" >&2
...
if [[ -n "$(git -C "$AGENT_ROOT" status --porcelain -- hermes_cli/env_loader.py)" ]]; then
  echo "FAIL_CLOSED: hermes_cli/env_loader.py has pre-existing uncommitted changes" >&2
  git -C "$AGENT_ROOT" status --short -- hermes_cli/env_loader.py >&2
  exit 1
fi
