...
assert(rendered.includes("COMMENTS RECEIVED (1)"), "received comment badge missing");
...
assert(nodes(commentedIdentity, (node) => String(node.props?.className || "") === "git-comments-issue-meta" && text(node).includes("COMMENTS RECEIVED (1)") && !text(node).includes("1 RECEIVED")).length === 1, "received must live inside the comments pill without redundant text");
