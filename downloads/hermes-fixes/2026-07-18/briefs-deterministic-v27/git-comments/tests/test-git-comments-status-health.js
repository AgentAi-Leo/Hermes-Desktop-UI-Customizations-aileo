...
assert(source.includes('.git-comments-success{position:fixed;left:50%;top:50%;z-index:1100;width:max-content;max-width:min(720px,calc(100vw - 64px));transform:translate(-50%,-50%);') && source.includes('padding:15px 24px') && source.includes('font-size:24px') && source.includes('border-radius:13.5px') && source.includes('transition:opacity .5s ease'), "all completion popups must share one exact center-screen position, stay below card width, and be 50% larger than Revision 38");
...
assert(source.includes('.git-comments-success.cyan{border-color:#22d3ee;background:#083344;color:#cffafe}'), "archive success must retain cyan styling");
assert(source.includes('.git-comments-success.red{border-color:#ef4444;background:#4a151b;color:#fecaca}'), "delete success must use red styling");
