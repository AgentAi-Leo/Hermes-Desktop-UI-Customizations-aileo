...
PACKAGE_NAME="git-watch-r61-content-fit-confirmations-v27"
...
restore_failure() {
  ...
  if [[ "$TOUCHED" == "1" ]]; then
    if ! (cd "$BACKUP" && shasum -a 256 -c BACKUP-CHECKSUMS.sha256 >/dev/null); then
      echo "GIT_WATCH_R61_RESTORE_FAILED=backup-integrity" >&2
      return 1
    fi
    ...
    if [[ "$failed" != "0" ]]; then
      echo "GIT_WATCH_R61_RESTORE_FAILED=copy" >&2
      return 1
    fi
    echo "GIT_WATCH_R61_RESTORED_AFTER_FAILURE=$BACKUP" >&2
  fi
}
...
required_renderer=[
    ...
    'event.key !== "Tab"',
    '<meta name="git-watch-export-version" content="61">',
    '<meta name="git-watch-visual-baseline" content="61">',
    'Export format 61 · Visual baseline 61',
    '.git-comments-action-modal{width:max-content;max-width:calc(100vw - 64px);max-height:calc(100vh - 64px);overflow:auto;box-sizing:border-box;display:grid;grid-template-columns:max-content max-content;column-gap:24px;border:1px solid #475569;border-radius:25px;',
    ...
    '.git-comments-action-title{grid-column:1/-1;margin:0;color:#FFE6CB;font-size:78px;font-weight:800;letter-spacing:.04em;white-space:nowrap}',
    '.git-comments-action-message{grid-column:1;grid-row:2;margin:14px 0 24px;color:#cbd5e1;font-size:51px;line-height:1.2;font-weight:800;letter-spacing:.04em;white-space:nowrap}',
    '.git-comments-action-buttons{grid-column:2;grid-row:3;justify-self:start;display:flex;align-items:center;gap:24px}',
    ...
    'CONFIRMATION DIALOG TOKENS (Revision 61)',
...
print('GIT_WATCH_R61_INSTALL_MARKERS=PASS')
PY
...
echo "GIT_WATCH_R61_BACKUP_RESTORE=PASS"
...
if [[ "${GIT_WATCH_R61_PAUSE_AFTER_RUNTIME:-0}" != "0" ]]; then
  [[ -z "${GIT_WATCH_R61_PAUSE_MARKER:-}" ]] || : > "$GIT_WATCH_R61_PAUSE_MARKER"
  sleep "$GIT_WATCH_R61_PAUSE_AFTER_RUNTIME"
fi

if [[ "${GIT_WATCH_R61_FAIL_AT:-}" == "after-runtime" ]]; then
...
TOUCHED=0
trap - EXIT
echo "GIT_WATCH_R61_RUNTIME_INSTALL=PASS"
