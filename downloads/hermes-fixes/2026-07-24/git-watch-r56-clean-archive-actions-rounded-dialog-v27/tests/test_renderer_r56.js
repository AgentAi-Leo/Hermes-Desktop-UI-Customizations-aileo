...
assert(source.includes('<meta name="git-watch-export-version" content="56">'), 'export schema must advance to Revision 56');
assert(source.includes('<meta name="git-watch-visual-baseline" content="56">'), 'visual baseline must identify Revision 56');
assert(source.includes('Export format 56 · Visual baseline 56'), 'visible export provenance must identify Revision 56');
...
assert(source.includes('CONFIRMATION DIALOG TOKENS (Revision 56)'), 'offline guide must document the rounded Alumni Sans SC confirmation dialog');
...
assert(source.includes('// Archived restore/delete are intentionally selection-only; per-row mutation buttons were retired in Revision 56.'), 'renderer must document why archived per-row mutations are intentionally absent');
...
console.log('GIT_WATCH_R56_RENDERER_CONTRACT=PASS');
