...
assert(
  ...
  "Revision 61 must preserve the 78px confirmation-title text"
);
assert(
  source.includes(".git-comments-action-message{grid-column:1;grid-row:2;margin:14px 0 24px;color:#cbd5e1;font-size:51px"),
  "Revision 61 must preserve the 51px confirmation-message text"
);
...
assert(
  ...
  "Revision 61 must preserve confirmation-button geometry and label size"
);
...
assert(
  ...
  "Revision 61 must render DESELECT ALL as yellow"
);
assert(
  ...
  "Revision 61 must render UNARCHIVE SELECTED as green"
);
...
assert(
  ...
  "Revision 61 modal must shrink to content width, retain the 64px viewport cap, and use a two-column alignment grid"
);
assert(
  source.includes(".git-comments-action-title{grid-column:1/-1;margin:0;color:#FFE6CB;font-size:78px"),
  "Revision 61 title must span the content and action columns"
);
assert(
  source.includes(".git-comments-action-message{grid-column:1;grid-row:2;margin:14px 0 24px;color:#cbd5e1;font-size:51px"),
  "Revision 61 message must define the first max-content column"
);
assert(
  ...
  "Revision 61 buttons must begin immediately after the message column rather than at the panel edge"
);
assert(
  ...
  "Revision 61 mobile rule must retain content sizing instead of restoring a fixed modal width"
);
assert(
  !source.includes(".git-comments-action-modal{width:min(1440px"),
  "Revision 61 must retire the oversized 1440px confirmation width"
);
assert(
  !source.includes(".git-comments-action-buttons{display:flex;align-items:center;justify-content:flex-end"),
  "Revision 61 must retire far-edge button justification"
);
assert(
  ...
  "Revision 61 confirmation titles must never wrap"
);
assert(
  ...
  "Revision 61 confirmation messages must never wrap"
);
assert(
  ...
  "Revision 61 confirmation button labels must never wrap"
);

console.log("GIT_WATCH_R61_DIALOG_TEXT_SCALE=PASS");
console.log("GIT_WATCH_R61_DIALOG_BUTTON_SCALE=PASS");
console.log("GIT_WATCH_R61_BULK_ACTION_COLORS=PASS");
console.log("GIT_WATCH_R61_MOBILE_GEOMETRY=PASS");
console.log("GIT_WATCH_R61_SINGLE_LINE_CONFIRMATIONS=PASS");
