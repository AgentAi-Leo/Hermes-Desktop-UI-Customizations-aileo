...
assert(exportedHtml.includes(exportClone.outerHTML) && exportedHtml.includes('<meta name="git-watch-export-version" content="48">') && exportedHtml.includes('<meta name="git-watch-visual-baseline" content="47">'), "export must embed the exact current HTML snapshot, export format 48, and the locked Revision 47 visual baseline");
assert(exportedHtml.includes("<!-- GIT WATCH OFFLINE FILE GUIDE") && exportedHtml.includes("API-DEPENDENT CONTROLS OMITTED") && exportedHtml.includes("SUCCESS POPUP TOKENS (Revision 47)") && exportedHtml.includes("border radius: 25px") && exportedHtml.includes("minimum width: 654.0625px") && exportedHtml.includes("minimum height: 143px") && exportedHtml.includes("padding: 48.75px 78px") && exportedHtml.includes("text size remains: 30.55px"), "export must explain offline scope and the final Revision 47 popup tokens for future editing");
...
assert(!exportedHtml.includes('<meta name="git-watch-export-version" content="47">'), "export must not retain the superseded Revision 47 export schema marker");
...
assert(source.includes('.git-comments-success{position:fixed;left:50%;top:50%;z-index:1100;width:max-content;min-width:min(654.0625px,calc(100vw - 78px));max-width:calc(100vw - 78px);min-height:min(143px,calc(100vh - 78px));box-sizing:border-box;transform:translate(-50%,-50%);') && source.includes('display:flex;align-items:center;justify-content:center') && source.includes('padding:48.75px 78px') && source.includes('font-size:30.55px') && !source.includes('font-size:38.1875px') && source.includes('color:rgba(255,255,255,.9)') && source.includes('border:1px solid rgba(187,247,208,.9);border-radius:25px') && source.includes('white-space:nowrap') && source.includes('box-shadow:0 15.6px 41.6px rgba(0,0,0,.6),0 5.2px 15.6px rgba(0,0,0,.36),0 0 14px rgba(134,239,172,.22),inset 0 1px 0 rgba(255,255,255,.16)') && source.includes('backdrop-filter:blur(5.2px)') && source.includes('transition:opacity .5s ease'), "center popup must enlarge only its box tokens by exactly 25%, keep 30.55px text, and use a 25px rounded-rectangle radius");
...
assert(source.includes('.git-comments-success{position:fixed;left:50%;top:50%;z-index:1100;width:max-content;min-width:min(654.0625px,calc(100vw - 78px));max-width:calc(100vw - 78px);min-height:min(143px,calc(100vh - 78px));'), "success popup box geometry must be exactly 25% larger than Revision 46");
assert(source.includes('padding:48.75px 78px;border:1px solid rgba(187,247,208,.9);border-radius:25px;'), "success popup must use 25%-larger padding with a 25px rounded-rectangle radius and pale green edge");
assert(source.includes('font-size:30.55px;line-height:1.25;font-weight:800;text-align:center;white-space:nowrap;') && !source.includes('font-size:38.1875px'), "success text must remain exactly 30.55px while the box grows");
...
assert(!source.includes('min-width:min(523.25px,calc(100vw - 62.4px))') && !source.includes('padding:39px 62.4px') && !source.includes('border-radius:35px') && !source.includes('.git-comments-success{border-radius:999px') && !source.includes('.git-comments-success{border-radius:50%'), "all success popups must use Revision 47 box geometry and 25px rounded rectangles rather than pills");
