...
PACKAGE_NAME="git-watch-r53-fullscreen-bulk-actions-v27"
...
restore_failure() {
  ...
  if [[ "$TOUCHED" == "1" ]]; then
    if ! (cd "$BACKUP" && shasum -a 256 -c BACKUP-CHECKSUMS.sha256 >/dev/null); then
      echo "GIT_WATCH_R53_RESTORE_FAILED=backup-integrity" >&2
      return 1
    fi
    ...
    if [[ "$failed" != "0" ]]; then
      echo "GIT_WATCH_R53_RESTORE_FAILED=copy" >&2
      return 1
    fi
    echo "GIT_WATCH_R53_RESTORED_AFTER_FAILURE=$BACKUP" >&2
  fi
}
...
actual={str(path) for path in root.rglob('*') if path.is_file() and path.name != 'CHECKSUMS.sha256'}
...
required_renderer=[
    ...
    'mutate("/watchlist/bulk-archived", { action: dialog.action, ids: selectedArchivedIds })',
    '<meta name="git-watch-export-version" content="53">',
...
print('GIT_WATCH_R53_INSTALL_MARKERS=PASS')
PY
...
echo "GIT_WATCH_R53_BACKUP_RESTORE=PASS"
...
if [[ "${GIT_WATCH_R53_PAUSE_AFTER_RUNTIME:-0}" != "0" ]]; then
  [[ -z "${GIT_WATCH_R53_PAUSE_MARKER:-}" ]] || : > "$GIT_WATCH_R53_PAUSE_MARKER"
  sleep "$GIT_WATCH_R53_PAUSE_AFTER_RUNTIME"
fi

if [[ "${GIT_WATCH_R53_FAIL_AT:-}" == "after-runtime" ]]; then
...
TOUCHED=0
trap - EXIT
echo "GIT_WATCH_R53_RUNTIME_INSTALL=PASS"
