...
required = [
    ...
    'const showSuccess = (message, duration, tone = "green") => { Cuelume.play("success");',
    'const archivedAuthor = entry.presentation?.author || entry.snapshot?.author || hydratedIssue?.author || {}',
    'archivedAuthorLogin.toLowerCase() === String(owner || "").toLowerCase()',
