...
assert(nodes(commentedIdentity, (node) => String(node.props?.className || "") === "git-comments-issue-meta" && text(node).includes("COMMENTS (1)") && text(node).includes("1 received")).length === 1, "comments/received metadata must render on a second row under the issue identity");
