...
def validate_ai(payload: object) -> dict:
    ...
    if not isinstance(takeaways, list) or len(takeaways) != 7:
        raise ContractError("founder_takeaways must contain exactly seven items")
    if not isinstance(topics, list) or len(topics) != 7:
        raise ContractError("topics must contain exactly seven items")
    ...
    for index, item in enumerate(topics, 1):
        ...
        if item["number"] != index:
            raise ContractError("topic numbers must be exactly 1 through 7 in order")
...
def render_stock(payload: object) -> RenderedBrief:
    ...
    writer.writerow(["ticker", "current_price", "daily_change", "daily_change_pct"])
    for quote in data["quotes"]:
        writer.writerow([quote["ticker"], quote["current_price"], quote["daily_change"], quote["daily_change_percent"]])
