...
describe("persistent route fullscreen", () => {
  ...
  it("resets stale fullscreen-shell scroll after an iframe load and across two layout frames", () => {
    const shell = { scrollTop: 101.5, scrollLeft: 14 };
    ...
    settlePersistentFullscreenShellScroll(shell, requestFrame);
    expect(shell).toEqual({ scrollTop: 0, scrollLeft: 0 });

    shell.scrollTop = 101.5;
    frames.shift()?.(0);
    expect(shell).toEqual({ scrollTop: 0, scrollLeft: 0 });
  ...
  it("centralizes Brief and Enter fullscreen ownership in the persistent App shell", () => {
    ...
    expect(app).toContain("settlePersistentFullscreenShellScroll(shell)");
  ...
  it("fits fullscreen Briefs to the A/B presentation without clipping while leaving normal mode unscaled", () => {
    ...
    expect(app).toContain("if (!isPersistentFullscreen || !isPersistentBriefRoute) return;");
    expect(app).toContain("shell.scrollTop = 0;");
    expect(app).toContain('shell.addEventListener("scroll", lockFullscreenShellScroll');
    expect(app).toContain("requestAnimationFrame(lockFullscreenShellScroll)");
    expect(app).toContain('shell.removeEventListener("scroll", lockFullscreenShellScroll');
  });

  it("keeps GIT WATCH vertically scrollable in persistent fullscreen", () => {
    const app = readFileSync(new URL("../App.tsx", import.meta.url), "utf8");

    expect(app).toContain('const isPersistentBriefRoute = normalizedPath === "/briefs-ai" || normalizedPath === "/brief-stock";');
    expect(app).toContain('normalizedPath === THREE_GOLD_GIT_WATCH_PATH');
    expect(app).toContain('"h-dvh overflow-y-auto overflow-x-hidden bg-background-base p-0"');
    expect(app).not.toContain('isPersistentFullscreen &&\n                    "h-dvh overflow-hidden bg-background-base p-0"');
  });
});
